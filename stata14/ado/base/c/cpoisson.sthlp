...
{p2colset 5 21 23 2}{...}
{p2col :{manlink R cpoisson} {hline 2}}Censored Poisson regression{p_end}
{p2colreset}{...}
...
{synopt :{cmd:ll(}{it:#}|{varname}{cmd:)}}specify lower limit for left-censoring{p_end}
{synopt :{cmd:ul(}{it:#}|{varname}{cmd:)}}specify upper limit for right-censoring{p_end}
...
{cmd:ul(}{it:#}|{varname}{cmd:)} specify the lower and upper limits for
censoring, respectively.  You may specify one or both.  Observations with
{depvar} {ul:<} {cmd:ll()} are left-censored; observations with
{depvar} {ul:>} {cmd:ul()} are right-censored; and remaining observations
are not censored.
...
{pstd}
The following option is available with {opt cpoisson} but is not shown in the
dialog box:
