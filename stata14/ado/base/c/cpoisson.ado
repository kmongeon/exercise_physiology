...
/* check syntax for lower limit ll */
	tokenize `"`ll'"', parse(" ,")
	if (`"`2'"'!="") {
...
/* check syntax for upper limit ul */
	tokenize `"`ul'"', parse(" ,")
	if (`"`2'"'!="") {
...
/* parse from() and check initial values */
	...
	local nparamx = `nparamx'-`nomitted'	
...
/* estimation */
	...
	eret local footnote censobs_table e(N_unc) e(N_lc) e(N_rc)
