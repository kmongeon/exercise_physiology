...
{pin2}
    ...
    Stata can process if it has enough memory.  This is usually 2,147,483,647
    for Stata/SE and Stata/IC and is 281,474,976,710,655 for Stata/MP.
...
{cmd:c(max_matsize)} and {cmd:c(min_matsize)} each return a numeric scalar
    ...
    of {cmd:matsize}, the two values will be equal.  {cmd:c(matsize)},
    documented under {it:{help creturn##memory:Memory settings}} below,
    returns the current value of {cmd:matsize}.
...
{cmd:c(charlen)} returns a numeric scalar reporting the maximum length of
  a {help char:characteristic}.  This number is larger in Stata/IC, Stata/SE,
  and Stata/MP than in Small Stata.
...
{cmd:c(namelenbyte)} returns a numeric scalar equal to {ccl namelenbyte},
    which is the current length in bytes of names in Stata.

{phang}
{cmd:c(namelenchar)} returns a numeric scalar equal to {ccl namelenchar},
    which is the current length in Unicode characters of names in Stata.
...
{cmd:c(filedate)} returns a string containing the date and time the file in
    {cmd:c(filename)} was last saved, such as {cmd:"7 Jul 2014 13:51"}.
...
{cmd:c(rng)} returns a string containing the current {helpb set rng} setting.
    ...
    Mersenne Twister random-number generator; {cmd:"kiss32"}, which specifies
    to always use the 32-bit KISS (keep it simple stupid) random-number
    generator; or {cmd:"default"}, which specifies to let Stata choose between
    these random-number generators based on {help version:version control}.
    Stata's default random-number generator in the absence of version control
    and with {cmd:set rng default} is the 64-bit Mersenne Twister.  See
    {manhelp set_rng R:set rng}.

{phang}
{cmd:c(rng_current)} returns a string containing the random-number generator
    (RNG) currently in effect, either {cmd:"mt64"} or {cmd:"kiss"}, depending
    on the current {helpb set rng} setting.  If {cmd:set rng} is currently
    set to {cmd:"default"}, then {cmd:c(rng_current)} depends on the current
    user version.  See {helpb version:[P] version}.
