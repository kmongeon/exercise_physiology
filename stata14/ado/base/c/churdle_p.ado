...
program define churdle_p
	...
	if `"`scores'"' != "" {
		...
		quietly generate double `strin3' = .
		quietly generate double `strin4' = .
		quietly generate double `strin5' = .
		quietly generate double `strin6' = .
	...
	mata: _CHURDLE_predict_hurd("`yhatp'", `bound', `hlatent',           ///
	...
	if `options'== 0 {
		generate `typlist' `varlist' = `yhatp'
		label var `varlist'                                          ///
		"Conditional mean estimates of dependent variable" 
	}

	if "`residuals'" !="" {
		generate `typlist' `varlist' =  `e(depvar)'-`yhatp'
		label var `varlist' "Residuals" 
	}

	if "`xb'" !="" {
		if "`equation'" == "" {
			generate `typlist' `varlist' = `xbp'
			label var `varlist'                                  ///
			    "Linear prediction xb for equation `e(depvar)'"
		}
		else {
			_predict `typlist' `xbeq', xb equation(`equation')
			generate `typlist' `varlist' = `xbeq'
			label var `varlist'                                  ///
	...
	if ("`e'" !=""){
		quietly generate `typlist' `varlist' = `ep'
		label var `varlist'                                          ///
		"Conditional mean estimate of dependent variable in (a,b)" 
	}

	if "`pr'" !="" {
		generate `typlist' `varlist' = `prp'
		label var `varlist'                                          ///
		"Probability of dependent variable in (a,b)" 
	}

	if "`ystar'" !="" {
		generate `typlist' `varlist' = `ystarp'
		label var `varlist'                                          ///
	...
	if "`stdp'" !="" {
		...
		generate `typlist' `varlist' = `stdpp'
