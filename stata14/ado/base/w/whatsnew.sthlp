...
{phang}
{hi:Additions to Stata since release 14.0}
...
{title:Description}
    ...
    Update history:
            ...
            {hi:Stata 14.0 base    02apr2015}
            {hi:Stata 14.1 base    29oct2015}
            {hi:Stata 14.2 base    06sep2016}
            {hi:updated to         16mar2017}

{pstd}
This file records the additions and fixes made to Stata since the release of
version 14.0.  The end of this file provides links for earlier additions and
fixes.
...
{p 5 9 2}
5.  Mata function
    {helpb mf_pdf##des_table_cell_top_bwidth:setCellTopBorderWidth({it:i, j, sz})}
...
4.  {helpb sem_command:sem} has improved starting values logic for models with
    latent and observed exogenous variables when option {cmd:method(mlmv)} or
    {cmd:noxconditional} is specified.  The result of this is faster
    convergence of the fitted target model.
...
{p 5 9 2}
3.  {helpb poisson_postestimation##estat gof:estat gof}, after fitting a
...
17.  {helpb pca_postestimation##predict:predict} after {cmd:pca} is documented
     to accept {cmd:stub*} notation when no statistic is specified.  Instead,
     {cmd:predict} exited with error message "{err:invalid name}".  The
...
25.  (Windows) {helpb export excel} did not save Excel files with the
     ...
     beyond the {mansection I Glossaryplainascii:plain ASCII} range.  This has
     been fixed.
...
10.  {cmd:predict}, when used to compute marginal predictions after fitting a
     weighted multilevel model with {helpb gsem_predict##predict:gsem},
...
{p 5 9 2}
5.  {helpb mixed_postestimation##estatdf:estat df} with option
...
See {help whatsnew13to14}.
    ...
    {c LT}{hline 63}{c RT}
    {c |} {bf:this file}        Stata 14.0, 14.1, and 14.2   2015 to present {c |}
