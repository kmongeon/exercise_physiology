...
{p2colset 5 39 41 2}{...}
{p2col :{manlink R heckoprobit postestimation} {hline 2}}Postestimation tools
...
{cmd:predict}
{dtype}
{c -(}{it:stub}{cmd:*} | {it:newvar_reg newvar_sel newvar_1 ... newvar_h newvar_athrho}{c )-}
{ifin}
{cmd:,} {opt sc:ores}
...
{phang}
{opt outcome(outcome)}
specifies for which outcome the predicted probabilities are to be calculated.
{opt outcome()} should contain either one value of the dependent
variable or one of {opt #1}, {opt #2}, ...., with {opt #1} meaning the first
category of the dependent variable, {opt #2} meaning the second category, etc.
