*! version 1.0.5  23feb2017  
program define heckprobit, eclass byable(onecall) prop(svyb svyj svyr ml_score)
	version 13.0
	if _by() {
	...
	if "`s(exit)'" != "" {
		version 10: ereturn local cmdline `"heckprobit `0'"'
		exit
	}
...
program define Estimate, eclass byable(recall)
	...
	syntax [varlist(numeric default=none ts fv)] [pw iw fw] [if] [in], /*
		...
		*/ CRITTYPE(passthru) DOOPT VCE(passthru) lf2debug * ] 
...
capture noisily break {
		`vv' ///
		capture noi ml model lf2 heckprob_lf2() 	 /*
...
capture noisily break {
	`vv' ///
	capture noi ml model lf2`d' heckprob_lf2() /*
	...
	*/ `crittyp' `diparm'  userinfo(`inits') 
	...
	est local predict "heckprobit_p"
	est local cmd "heckprobit"
	est local marginsnomarkout PSel XBSel
	if ("`selname'" != "select") {
		est hidden local seldep `seldep'
	}
