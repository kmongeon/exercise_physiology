*! version 1.1.2  23feb2017
program define heckoprobit, eclass byable(onecall) ///
	properties(svyb svyj svyr fvaddcons)
	version 13.0
	if _by() {
...
program define Estimate, eclass byable(recall)
	...
	oprobit `depvar' `varlist' `wgt' if `touse' & `seldep' 	///
		,nocoef `offopt' `cns' nocnsnotes `coll' `vceopt' `log'
	...
	capture noisily break {
		...
		`log' `mlopts' `diparm' `coll'				///
		`vceopt' nopreserve userinfo(`inits')
	}	
	local erc = _rc
	...
	ereturn local marginsok		default		///
					...
					xb		///
					xbsel
	ereturn local marginsnomarkout	PSel XBSel
	...
	if (e(N_cens) != .) {
		ereturn scalar N_cens = e(N_cens)
	}
	if (e(N_cd) != .) {
