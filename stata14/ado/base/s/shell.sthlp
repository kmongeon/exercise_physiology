...
{pstd}
On Mac OS X, {opt xterm} is available when {it:X11} is installed.  {it:X11} is
installed by default on Mac OS X 10.7 but not on Mac OS X 10.8 and newer.  To
install {it:X11} on these newer operating systems, you must first download
XQuartz from {browse "http://xquartz.macosforge.org/"}.
