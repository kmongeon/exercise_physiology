...
{bf:complementary log-log regression}.
     ...
     response functions that are family Bernoulli, link cloglog.  It is used
     for binary outcome data.  Complementary log-log regression is also known
     in Stata circles as cloglog regression or just cloglog. See
...
{pmore}
        ...
        provided automatically by {cmd:sem} and {cmd:gsem} by 
        {help sem glossary##anchoring:anchoring} with 
        unit loadings. 
...
{pmore}
	...
	cloglog regression, and the like.  These parameters are also included
	in theta.
