...
program define slogit_p
	...
	if `"`scores'"' != "" {
        ...
        if `nopt' > 1 {
                di as error "{p}only one of {bf:pr}, {bf:xb}, {bf:index}, " ///
		 "{bf:stdp}, or {bf:seindex} can be specified{p_end}"
                exit 198
        }
	if "`equation'" != "" {
		if "`outcome'" != "" {
			di as error "{p}only one one of {bf:outcome()} or " ///
			 "{bf:equation()} can be specified{p_end}"
			exit 198
		}
	 ...
	 | "`type'"=="stdp" | "`type'"=="seindex" {
		...
		if `nvar' > 1 {
			di as error "{p}only one new variable can be " ///
 			 "specified with {bf:`type'} option{p_end}"
			exit 103
		}
		if index("`vlist'","*") {
