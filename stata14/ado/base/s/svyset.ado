...
program Get, rclass
	...
	if `stages_wt' {
		if "`return(wtype)'" != "" {
			...
			di as err ///
"`return(wtype)'s not allowed with stage-level weight() options"
			exit 198
		}
		if "`return(brrweight)'" != "" {
			di as err "invalid weight specification;"
			di as err ///
"option brrweight() not allowed with stage-level weight() options"
			exit 198
		}
		if "`return(bsrweight)'" != "" {
			di as err "invalid weight specification;"
			di as err ///
"option bsrweight() not allowed with stage-level weight() options"
			exit 198
		}
		if "`return(jkrweight)'" != "" {
			di as err "invalid weight specification;"
			di as err ///
"option jkrweight() not allowed with stage-level weight() options"
			exit 198
		}
		if "`return(sdrweight)'" != "" {
			di as err "invalid weight specification;"
			di as err ///
"option sdrweight() not allowed with stage-level weight() options"
			exit 198
		}
	}
	if `stages_wt' > `stages' {
