...
{p2colset 5 28 30 2}{...}
{p2col :{manlink TE stteffects wra} {hline 2}}Survival-time weighted
...
{synopt :{cmd:ancillary(}{it:avarlist}[{cmd:,} {opt nocon:stant}]}specify
variables used to model ancillary parameter{p_end}
{synopt :{opt nocon:stant}}suppress constant from outcome model{p_end}
{synoptline}
...
{synopt :{opt gam:ma}}two-parameter gamma{p_end}
{synopt :{opt ln:ormal}}lognormal{p_end}
{synopt :{cmd:ancillary(}{it:avarlist}[{cmd:,} {opt nocon:stant}]}specify variables used to model ancillary parameter{p_end}
{synopt :{opt nocon:stant}}suppress constant from censoring model{p_end}
{synoptline}
...
for
the model for the censoring variable, or for both.  If {cmd:ancillary()} is
specified for both, the varlist used for each model may be different.
