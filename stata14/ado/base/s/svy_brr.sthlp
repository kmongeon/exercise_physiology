...
command.  The command is executed once for each replicate using sampling
weights that are adjusted according to the BRR methodology.
Any Stata estimation command listed in 
{manhelp svy_estimation SVY:svy estimation} may be used with {cmd:svy}
{cmd:brr}.
User-written commands that meet the requirements in
