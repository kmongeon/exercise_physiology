...
*/ "(type {cmd:ssc describe} {it:pkgname} for more information on {it:pkgname})"
		...
		if _rc==601 | _rc==661  {
			di as err /*
*/ `"ssc describe: "`pkgname'" not found at SSC, type {stata search `pkgname'}"'
			di as err /*
...
program define sscinstall
	* ssc install <package> [, <net_install_options>]
	...
	if _rc==601 | _rc==661 {
		di as err /*
*/ `"ssc install: "`pkgname'" not found at SSC, type {stata search `pkgname'}"'
		di as err /*
*/ "(To find all packages at SSC that start with `ltr', type {stata ssc describe `ltr'})"
		exit `rc'
	}
...
program define ssccopy
	...
	* backwards compatibility: sjplus and stbplus are synonyms for plus
	...
	if _rc==601 | _rc==661 {
		di as err /*
	*/ `"ssc copy: "`fn'" not found at SSC, type {stata search `fn'}"'
		exit `rc'
	}
	if _rc {
		error `rc'
