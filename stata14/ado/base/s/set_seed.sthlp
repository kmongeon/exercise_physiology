...
{p2colset 5 21 23 2}{...}
{p2col :{manlink R set seed} {hline 2}}Specify random-number seed and state{p_end}
{p2colreset}{...}
...
Remarks are presented under the following headings:
	...
	{help set_seed##rngs:Random-number generators in Stata}
...
{it:{help set_seed##rngs:Random-number generators in Stata}} below for more details.
...
{pstd}
The default pseudorandom-number generator introduced in Stata 14 is the 64-bit
Mersenne Twister.  See {help set seed##MN1998:Matsumoto and Nishimura (1998)}
for more details.  The default pseudorandom-number generator in Stata 13 and
earlier versions is George Marsaglia's 32-bit KISS generator (G. Marsaglia,
1994, pers. comm.)  Both generators are available in Stata 14. The
abbreviations {cmd:mt64} and {cmd:kiss32} are used, respectively, to specify
these two generators in Stata commands and functions. 
 
{pstd} You can specify the random-number generator you want to use in three
ways: 1) through version control; 2) with the {cmd:set rng} command; and 3)
through functions and system parameters explicitly named after the generators.
We now elaborate on each of these methods.

{p 8 12 2}
    1.  By default, Stata will use the {cmd:mt64} generator or the {cmd:kiss32}
	generator based on version control.  See {manhelp version P} for more
	details.

{p 8 12 2}
    2.  If you wish to always use only the {cmd:mt64} generator or only the
	{cmd:kiss32} generator, you can use {cmd:set rng} to explicitly state
	which random-number generator you wish to use.  The scope of this
	setting is the Stata session, do-file, or program in which {cmd:rng}
	is set.  See {manhelp set_rng R:set rng} for more details.

{p 8 12 2}
    3.  Finally, all random-number functions have variants that are explicitly
	named after each generator, using the generator abbreviation as the
	suffix.  For example, {cmd:runiform_mt64()}, {cmd:runiform_kiss32()}, 
	{cmd:rnormal_mt64()}, {cmd:rnormal_kiss32()}, etc. 

{p 12 12 2}
       The system parameters {cmd:seed} and {cmd:rngstate} also have variants
       explicitly named after each generator: {cmd:seed_mt64},
       {cmd:seed_kiss32}, {cmd:rngstate_mt64}, and {cmd:rngstate_kiss32}. 

{p 12 12 2}
	For example, here is how you can set the seed, generate a random
	number, preserve a state, and restore a state for {cmd:mt64}:
...
{p 12 12 2}
	Similarly, for {cmd:kiss32}, the commands are 

{p 12 12 2}
       {cmd:. set seed_kiss32 524091}

{p 12 12 2}
       {cmd:. generate l = rlogistic_kiss32()}

{p 12 12 2}
       {cmd:. local state = c(rngstate_kiss32)}

{p 12 12 2}
       {cmd:. set rngstate_kiss32 `state'}
