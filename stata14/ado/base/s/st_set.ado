...
*/
	...
	di in ye %11.0g round(r(sum),0.001) in gr /*
	*/ "  total analysis time at risk and under observation"
     	di in gr _col(49) "at risk from t = " in ye %9.0g 0
	drop `wrk'
	qui sum _t0 if _st, meanonly
	di in gr _col(38) "earliest observed entry t = " in ye %9.0g r(min)
	qui sum _t if _st, meanonly
	di in gr _col(43) "last observed exit t = " in ye %9.0g r(max)
end
...
program define Di1
	di in ye %11.0g round(`1',0.001) in gr `"  `2'"'
	mac shift 2 
	while `"`1'"' != "" {
		di in gr _skip(13) `"`1'"'
		mac shift 
...
program define Di2h
	if (`1')==0 { exit }
	di in ye %11.0g round(`1',0.001) in gr `"  `2'"' in gr " `3'" /* 
...
program define Di2z
	di in ye _skip(13) %9.0g (`1') in gr `" `2'"'
	mac shift 2 
	while `"`1'"' != "" {
