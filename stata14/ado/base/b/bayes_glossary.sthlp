...
{phang}
{bf:Bayes's rule}.
The Bayes's rule is a formal method for relating conditional probability
statements.  For two (random) events X and Y, the Bayes's rule states that
...
analysis, the Bayes's rule is used for combining prior information about model
parameters and evidence from the observed data to form the
...
{help bayes_glossary##Bayes_rule:Bayes's rule}, the prior distribution and the
likelihood model are combined to form the posterior distribution of model
parameters.  The posterior distribution is then used for parameter inference,
...
{it:{mansection BAYES bayesmhpostestimationRemarksandexamplesDifferentwaysofspecifyingmodelparameters:Different ways of specifying model parameters}} in {manhelp bayesmh_postestimation BAYES:bayesmh postestimation}.
...
A posterior predictive distribution is a distribution of unobserved (future)
data conditional on the currently observed data.  Posterior predictive
distribution is derived by marginalizing the likelihood function with respect
to the posterior distribution of model parameters.
...
Parameters associated with a random-effects variable.  Random-effects
parameters are assumed to be conditionally independent across levels of the
random-effects variable given all other model parameters.  Often,
random-effects parameters are assumed to be normally distributed with a zero
