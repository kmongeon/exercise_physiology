...
{p2colset 5 24 26 2}{...}
{p2col :{manlink BAYES bayesmh} {hline 2}}Bayesian regression using Metropolis-Hastings algorithm{p_end}
{p2colreset}{...}
...
use {it:param}{cmd:=}{it:#} to specify an initial value.  For
example, you can specify, {cmd:{c -(}}{cmd:var=1}{cmd:{c )-}},
...
{syntab:Multivariate continuous}
...
{synopt :{opt block}{cmd:(}{it:{help bayesmh##paramref:paramref}}[{cmd:,} {it:{help bayesmh##blockopts:blockopts}}]{cmd:)}}specify a block of model parameters; this option may be repeated{p_end}
{synopt :{opt init:ial}{cmd:(}{it:{help bayesmh##initspec:initspec}}{cmd:)}}initial values for model parameters {p_end}
{synopt :{opt nomleinit:ial}}suppress the use of maximum likelihood estimates as starting values{p_end}
{synopt :{opt initrand:om}}specify random initial values{p_end}

{syntab :Simulation}
{synopt :{opt mcmcs:ize(#)}}MCMC sample size; default is {cmd:mcmcsize(10000)}{p_end}
{synopt :{opt burn:in(#)}}burn-in period; default is {cmd:burnin(2500)}{p_end}
{synopt :{opt thin:ning(#)}}thinning interval; default is {cmd:thinning(1)}{p_end}
{synopt :{opt rseed(#)}}random-number seed{p_end}
{synopt :{opt excl:ude}{cmd:(}{it:{help bayesmh##paramref:paramref}}{cmd:)}}specify model parameters to be excluded from the simulation results{p_end}

{syntab: Adaption}
{synopt :{opt adapt:ation}{cmd:(}{it:{help bayesmh##adaptopts:adaptopts}}{cmd:)}}control the adaptive MCMC procedure{p_end}
{synopt :{opt sc:ale(#)}}initial multiplier for scale factor; default is {cmd:scale(2.38)}{p_end}
{synopt :{opt cov:ariance(cov)}}initial proposal covariance; default is the identity matrix{p_end}
...
{synopt :{opt noexpr:ession}}suppress output of expressions from model summary{p_end}
{synopt :{opt blocksumm:ary}}display block summary{p_end}
{synopt :{opt dot}}display dots every 100 iterations and iteration numbers
every 1,000 iterations{p_end}
{synopt :{cmd:dots(}{it:#}[{cmd:,} {opt every(#)}]{cmd:)}}display dots as simulation is performed{p_end}
{synopt :{opth noshow:(bayesmh##paramref:paramref)}}specify model parameters to be excluded from the output{p_end}
{synopt :{opth showre:ffects(bayesmh##paramref:paramref)}}specify random-effects parameters to be included in the output{p_end}
...
{synopt :{cmdab:sav:ing(}{it:{help filename}}[{bf:, replace}]{bf:)}}save simulation results to {it:filename}{cmd:.dta}{p_end}
{synopt :{it:{help bayesmh##display_options:display_options}}}control
spacing, line width, and base and empty cells{p_end}

{syntab: Advanced}
{synopt :{opt search}{cmd:(}{it:{help bayesmh##search_options:search_options}}{cmd:)}}control the search for feasible initial values{p_end}
...
{p 4 6 2}Options {cmd:prior()}, {cmd:redefine()}, and {cmd:block()} can be repeated.{p_end}
...
the command).{p_end}
{p 4 6 2}See {manhelp bayesmh_postestimation BAYES:bayesmh postestimation} for
...
{synopt :{opt cov:ariance(#)}}initial proposal covariance for the current
...
{phang}
{bf:Statistics > Bayesian analysis > Estimation}
...
{pmore}
{it:{help bayesmh##modelspec:modelspec}} specifies one of the supported likelihood
distributions for regression models.  A location parameter of these
...
{marker exposure}{...}
{phang2}
{opth exposure:(varname:varname_e)} specifies a variable that reflects the amount of
exposure over which the {depvar} events were observed for each observation;
ln({it:varname_e}) with coefficient constrained to be 1 is entered into
the log-link function. This option is available with {cmd:poisson}.
...
specifications of the model before fitting the model.
...
models and can be repeated.  See {mansection BAYES bayesmhRemarksandexamplesex29:example 29}.
...
{phang}
{cmd:block(}{it:{help bayesmh##paramref:paramref}}[{cmd:,}
{it:blockopts}]{cmd:)} specifies a group of model parameters for the blocked
MH algorithm.  By default, all parameters except matrices are treated as
one block, and each matrix parameter is viewed as a separate block.  You can
use the {cmd:block()} option to separate scalar parameters in multiple blocks.
Technically, you can also use {cmd:block()} to combine matrix parameters in
one block, but this is not recommended.  The {cmd:block()} option may be
repeated to define multiple blocks.  Different types of model parameters, such
as scalars and matrices, may not be specified in one {cmd:block()}.
Parameters within one block are updated simultaneously, and each block of
parameters is updated in the order it is specified; the first specified block
is updated first, the second is updated second, and so on. See
...
{phang2}
{it:blockopts} include {cmd:gibbs}, {cmd:split}, {cmd:reffects}, {cmd:scale()},
{cmd:covariance()}, and {cmd:adaptation()}.

{phang2}
{cmd:gibbs} option specifies to use Gibbs sampling to update parameters in the
block.  This option is allowed only for specific combinations of likelihood
models and prior distributions; see
...
in {bf:[BAYES] bayesmh}.
{cmd:gibbs} may not be combined with {cmd:reffects}, {cmd:scale()},
{cmd:covariance()}, or {cmd:adaptation()}.
...
{cmd:reffects} requires that parameters be specified as 
...
See {mansection BAYES bayesmhRemarksandexamplesex25:example 25} for details.
...
parameters in the block.  By default, {it:#} is equal to 2.38 (that is,
{cmd:scale(2.38)}) is the default.  If specified, this option overrides the
respective setting from the {cmd:scale()} option specified with the
{cmd:bayesmh} command.  {cmd:scale()} may not be combined with {cmd:gibbs}.
...
specified block.  The initial proposal covariance is computed as
{it:rho} x {it:Sigma}, where {it:rho} is a scale factor and
{it:Sigma} = {it:matname}.  By default, {it:Sigma} is the identity
matrix.  If specified, this option overrides the respective setting from the
{cmd:covariance()} option specified with the {cmd:bayesmh} command.
{cmd:covariance()} may not be combined with {cmd:gibbs}.

{phang2}
{cmd:adaptation(tarate())} and {cmd:adaptation(tolerance())} specify
block-specific TAR and acceptance tolerance. If specified,
they override the respective settings from the {cmd:adaptation()} option
specified with the {cmd:bayesmh} command.
{cmd:adaptation()} may not be combined with {cmd:gibbs}.
...
{phang3}
{cmd:bayesmh} ..., {cmd:initial({c -(}{cmd:alpha}{c )-}}{cmd:0.5}{cmd:{c -(}}{cmd:Sigma,m{c )-}} {cmd:I(2))} ...

{pmore}
You can also specify a list of parameters using any of the specifications
described in
{help bayesmh##refer:{it:Referring to model parameters}}.
For example, to initialize all regression coefficients from equations {cmd:y1}
and {cmd:y2} to zero, you can type

{phang3}
{cmd:bayesmh} ...{cmd:,} {cmd:initial({c -(}}{cmd:y1:}{cmd:{c )-}} {cmd:{c -(}y2:{c )-}} {cmd:0)} ...

{pmore}
The general specification of {it:initspec} is

{phang3}
{it:{help bayesmh##paramref:paramref}} {it:#} [{it:paramref} {it:#} [...]]
...
{phang}
{cmd:nomleinitial} suppresses using maximum likelihood estimates (MLEs)
starting values for regression coefficients.  By default, when no initial
values are specified, MLE values (when available) are used as initial
values.  If {cmd:nomleinitial} is specified and no initial values are
provided, {cmd:bayesmh} uses ones for positive scalar parameters, zeros for
...
{phang}
{cmd:initrandom} requests that the model parameters be initialized randomly.
...
parameters, you can specify them in the {cmd:initial()} option or during
parameter declarations in the {cmd:likelihood()} option.  Random initial
values are not available for parameters with {cmd:flat}, {cmd:density()},
{cmd:logdensity()}, and {cmd:jeffreys()} priors; you must provide fixed
...
than the adaptation period. Also see
{mansection BAYES bayesmhRemarksandexamplesBurn-inperiodandMCMCsamplesize:{it:Burn-in period and MCMC sample size}}
and
...
values from every (1+k x {cmd:#})th iteration for k = 0, 1, 2, ... are saved
in the final MCMC sample; all other simulated values are discarded. The
default is {cmd:thinning(1)}; that is, all simulation values are saved.
Thinning greater than one is typically used for decreasing the autocorrelation
of the simulated MCMC sample.
...
{cmd:seed} {it:#} prior to calling {cmd:bayesmh}; see
...
{mansection BAYES bayesmhRemarksandexamplesAdaptationoftheMHalgorithm:{it:Adaptation of the MH algorithm}}
...
k x k covariance matrix.  If the adaptation interval is not sufficient
...
computed as max{c -(}25,{cmd:floor(burnin()/adaptation(every()))}{c )-}.

{pmore3}
{opt maxiter()} is usually chosen to be no greater than
({cmd:mcmcsize()}+{cmd:burnin()})/{cmd:adaptation(every())}.

{phang3}
{opt miniter(#)} specifies the minimum number of adaptive iterations
to be performed regardless of whether the TAR has been
achieved.  The default is {cmd:miniter(5)}.  If the specified {cmd:miniter()}
is greater than {cmd:maxiter()}, then {cmd:miniter()} is reset to
{cmd:maxiter()}.  Thus, if you set {cmd:maxiter(0)}, then no adaptation will
be performed.

{phang3} 
{opt alpha(#)} specifies a parameter controlling the adaptation of the
AR.  {cmd:alpha()} should be in [0,1]. The default is {cmd:alpha(0.75)}.

{phang3}
{opt beta(#)} specifies a parameter controlling the adaptation of the
proposal covariance matrix.  {cmd:beta()} must be in [0,1].  The closer
{cmd:beta()} is to zero, the less adaptive the proposal covariance.  When
{cmd:beta()} is zero, the same proposal covariance will be used in all MCMC
iterations.  The default is {cmd:beta(0.8)}.

{phang3}
{opt gamma(#)} specifies a parameter controlling the adaptation rate
of the proposal covariance matrix.  {cmd:gamma()} must be in [0,1].  The
larger the value of {cmd:gamma()}, the less adaptive the proposal covariance.
The default is {cmd:gamma(0)}.

{phang3}
{opt tarate(#)} specifies the TAR for all blocks of model parameters; this is
rarely used.  {cmd:tarate()} must be in (0,1). The default AR is 0.234 for
blocks containing continuous multiple parameters, 0.44 for blocks with one
continuous parameter, and 1/{it:n_maxlev} for blocks with discrete parameters,
where {it:n_maxlev} is the maximum number of levels for a discrete parameter
in the block.

{phang3}
{opt tolerance(#)} specifies the tolerance criterion for adaptation
based on the TAR.  {cmd:tolerance()} should be in (0,1).
Adaptation stops whenever the absolute difference between the current and
TARs is less than {cmd:tolerance()}.  The default is {cmd:tolerance(0.01)}.
...
where n_p is the number of parameters in the block.  By default,
{it:#} is equal to 2.38; that is, {cmd:scale(2.38)} is the default.
...
INCLUDE help bayes_clevel_hpd.ihlp

INCLUDE help bayes_batch.ihlp

{phang}
{opt nomodelsummary} suppresses the detailed summary of the specified model.
Model summary is reported by default.
...
is useful when {cmd:block()} is specified and may not be combined with
{cmd:dryrun}.

{phang}
{opt dots} and {opt dots(#)} specify to display dots as simulation is
performed.   {opt dots(#)} displays a dot every {it:#} iterations.  During the
adaptation period, a symbol {cmd:a} is displayed instead of a dot. If
{cmd:dots(}...{cmd:,} {opt every(#)}{cmd:)} is specified, then an iteration
number is displayed every {it:#}th iteration instead of a dot or {cmd:a}.
{cmd:dots(,} {opt every(#)}{cmd:)} is equivalent to
{cmd:dots(1,} {opt every(#)}{cmd:)}.  {cmd:dots}
displays dots every 100 iterations and iteration numbers every 1,000
iterations; it is a synonym for {cmd:dots(100),} {cmd:every(1000)}. By
default, no dots are displayed ({cmd:dots(0)}).

{phang}
{opth noshow:(bayesmh##paramref:paramref)} specifies a list of model
parameters to be excluded from the output.  Do not confuse this option with
{cmd:exclude()}, which excludes the specified parameters from the MCMC
sample.

{phang}
{opth showreffects:(bayesmh##paramref:paramref)} is used with option 
{cmd:reffects()} and specifies a list of random-effects parameters to be 
included in the output.  By default, all random-effects parameters introduced by 
...
in the {cmd:exclude()} and {cmd:noshow()} options.  Regression model
parameters are grouped by equation names.  The table includes six columns and
reports the following statistics using the MCMC simulation results: posterior
mean, posterior standard deviation, MCMC standard error or MCSE, posterior
...
overwrite {it:filename}{cmd:.dta} if it exists. If the {cmd:saving()} option
...
state in variable {cmd:_frequency}. (Some states may be repeated for discrete
parameters.) As such, {cmd:_index} may not necessarily contain consecutive
integers. Remember to use {cmd:_frequency} as a frequency weight if you need
to obtain any summaries of this dataset.  Values for each parameter are saved
in a separate variable in the dataset.  Variables containing values of
parameters without equation names are named as {cmd:eq0_p}{it:#}, following
the order in which parameters are declared in {cmd:bayesmh}.  Variables
containing values of parameters with equation names are named as
{cmd:eq}{it:#}{cmd:_p}{it:#}, again following the order in which parameters
are defined.  Parameters with the same equation names will have the same
variable prefix {cmd:eq}{it:#}.  For example,
...
{cmd:eq0_p1} for {cmd:{c -(}var{c )-}}.  Also see macros {cmd:e(parnames)} and
{cmd:e(varnames)} for the correspondence between parameter names and variable
names.
...
{cmd:_logposterior} to contain values of log posterior from each iteration.

INCLUDE help bayes_display.ihlp
...
{cmd:search_options} are {cmd:on}, {opt repeat(#)}, and {cmd:off}.
...
{phang2}
{cmd:search(off)} prevents {cmd:bayesmh} from searching for feasible initial
...
{mansection BAYES bayesmhpostestimationRemarksandexamplesDifferentwaysofspecifyingmodelparameters:{it:Different ways of specifying model parameters}}
in {bf:[BAYES] bayesmh postestimation}.
...
2. Linear combinations can be specified using the notation
         {cmd:{c -(}}{it:eqname}{cmd::}{it:varlist}{cmd:{c )-}}. For example,

{p 7 7 2}{cmd:{c -(}}{cmd:xb:mpg price weight{c )-}} is equivalent to 

{phang3}{cmd:{xb_mpg}*mpg} {cmd:+} {cmd:{xb_price}*price} +
                    {cmd:{xb_weight}*weight}

{p 4 7 2}
3. There is a small caveat with using the
	 {cmd:{c -(}}{it:eqname}{cmd::}{it:name}{cmd:{c )-}} specification 2
	 when {it:name} corresponds to both one of the variables in the
	 dataset and a parameter in the model.  The linear-combination
	 specification takes precedence in this case.  For example,
	 {cmd:{eq:var}} will be expanded to {cmd:{eq_var}*var}, where
	 {cmd:var} is the variable in a dataset and {cmd:eq_var} is the
	 coefficient corresponding to this variable.  To refer directly to the
	 coefficient, you must use {cmd:{eq_var}}.

{p 4 7 2}
4. Initial values are given by including an equal sign and the initial
...
{synopt:{cmd:e(batch)}}batch length for batch-mean calculations{p_end}
{synopt:{cmd:e(corrlag)}}maximum autocorrelation lag{p_end}
{synopt:{cmd:e(corrtol)}}autocorrelation tolerance{p_end}
{synopt:{cmd:e(dic)}}deviation information criterion{p_end}
...
{synopt:{cmd:e(block}{it:#}{cmd:_arate_last)}}{it:#}th block AR from the last adaptive iteration{p_end}
...
{synopt:{cmd:e(rngstate)}}random-number state at the time of simulation{p_end}
