*! version 1.1.17  28mar2016
program betareg, eclass byable(onecall) properties(swml svyb svyj svyr)
	version 14.0
	if _by() {
...
program Estimate, eclass byable(recall) 
	...
	ereturn hidden local linkf `linkf'
	ereturn hidden local linkt `linkt'
	ereturn hidden local slinkf `slinkf'
	ereturn hidden local slinkt `slinkt'
...
program define StartingValues, rclass
	...
	if `"`link'"'=="logit" {
		...
		qui gen double  `mu'= 1 - exp(-exp(`yt_p'))
	}
	else if `"`link'"'=="loglog" {
