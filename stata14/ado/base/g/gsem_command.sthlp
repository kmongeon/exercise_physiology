...
{p2colset 5 19 21 2}{...}
{p2col:{manlink SEM gsem} {hline 2}}Generalized structural equation model
...
{phang}
3. {cmd:gsem} allows Stata's factor-variable notation to be used
          in specifying models, something {cmd:sem} does not.

{phang}
4. {cmd:gsem}'s method ML
...
{phang}
5. {cmd:gsem} lacks the group-comparison capabilities
          that {cmd:sem} provides.
...
{phang}
3. {cmd:gsem} deletes options related to features it does not have,
          such as group-comparison capabilities and SSDs.

{phang}
4. {cmd:gsem} adds technical options for controlling
...
{phang2}{cmd:. gen selected = wage < .}{p_end}
