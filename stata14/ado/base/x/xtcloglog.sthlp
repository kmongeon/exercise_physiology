...
{viewerjumpto "Menu" "xtcloglog##menu"}{...}
...
{viewerjumpto "Stored results" "xtcloglog##results"}{...}
{title:Title}
...
[{it:{help xtcloglog##weight:weight}}]
[{cmd:, re}
...
{p 8 18 2}
{cmd:xtcloglog} {depvar} [{indepvars}] {ifin}
[{it:{help xtcloglog##weight:weight}}]{cmd:, pa}
...
{syntab:Correlation}
{synopt :{cmdab:c:orr(}{it:{help xtcloglog##correlation:correlation}}{cmd:)}}within-panel correlation structure{p_end}
...
log-log (cloglog) models for a binary dependent variable.  Complementary
log-log models are typically used when one of the outcomes is rare relative to
the other. 
...
{mansection XT xtcloglogMethodsandformulasxtcloglog,reandtherobustVCEestimator:{it:xtcloglog, re and the robust VCE estimator}} in
{it:Methods and formulas} of {bf:[XT] xtcloglog}.
...
{pstd}
{cmd:xtcloglog, re} stores the following in {cmd:e()}:
...
{pstd}
{cmd:xtcloglog, pa} stores the following in {cmd:e()}:
