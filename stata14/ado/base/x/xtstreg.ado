*! version 1.0.4  16dec2015
program xtstreg, eclass byable(onecall)
	version 14
...
program Estimate, eclass sortpreserve byable(recall)
	...
	syntax [varlist(ts fv default=none)] [if] [in] [fw pw iw] ,	///
		Distribution(passthru) [ TIme OFFset(varname) 		///
		noCONstant INTMethod(string) INTPoints(integer 12) 	///
		vce(passthru) I(string) from(passthru) noSKIP noDISplay	///
		noSHow noHR TRatio *]
	...
	if "`from'" != "" | missing("`xvars'") local skip
	if "`skip'" != "" {
		if "`clust'`robus'" != "" {
	...
	ereturn local title "Random-effects `e(model)' regression"
