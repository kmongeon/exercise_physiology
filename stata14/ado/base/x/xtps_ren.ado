...
exit 198
	...
	quietly {
		...
		if "`constan'" == "" {
			local names "`names' `odep':_cons lnsig2u:_cons"
		}
		else {
	...
	if "`skip'" == "" & "`ind'" != "" {
		...
		`lll' ml model d2 xtpoisson_d2 (`dep'=, `constan' `oarg')  ///
			...
			`nolog' max `mlopt' `mycns' search(off) nopreserve ///
	...
	/// Calculate logF (by panel) used to prevent underflow
	quietly {
		`vv' mat colnames `beta' = `ind' _cons
	...
	`lll' ml model d2 xtpoisson_d2 (`depstr':`dep'=`ind', 	///
		...
		`log' max `mlopt' `mycns' search(off) nopreserve `negh' ///
