...
{pstd}
{cmd:version} with no arguments shows the current version number to which the
command interpreter is set.  It can be used interactively, in do-files, or in
ado-files. 

{pstd}
{cmd:version} {it:#} sets the command interpreter to internal version number
{it:#} -- it makes Stata understand old, out-of-date syntaxes.  It makes old
programs run correctly in modern Statas.  If you put {cmd:version} {it:#} at
the top of your new do- and ado-files, it will ensure that the code you write
today will continue to work in the future. 

{pstd}
{cmd:version} can also be used as a prefix.
{cmd:version} {it:#}{cmd::} {it:command}
executes {it:command} under version {it:#}.
The version subsequently remains unchanged. 
...
{pmore}
If you type {cmd:version} {it:#} interactively or in your do-files, 
Stata not only understands old syntax, it backdates (removes) improvements
made after {it:#}, such as those to the RNGs.  You do not have to specify 
the {cmd:user} option.  The modern version of Stata will still produce 
the same results as it produced in the past. 
