...
void unicode_do(`ProblemS' pr) 
{
	...
	if (pr.retranslate | pr.transutf8) {
		do_undo_translations(pr, pr.retranslate, pr.replace)
		pr.filestatus = J(1, 0, .) 
		get_filestatus(pr, `False') 
	}
	if (pr.reanalyze) {
...
void do_undo_translations(`ProblemS' pr, `booleanS' retrans, `boolean' replace)
{
	`RetcodeS'	rc
	`RS'		i
			...
			if(rc == 0) {
				...
				if(maxvar < kvar) {
					...
					if(isse) {
						...
						printf("File can not be processed by Intercooled or Small Stata.\n")						
					}
					printf("{p_end}\n")
				}
			}
...
`TranscodeS' examine_txt_file(`ProblemS' pr, `SS' fn) 
{
	...
	if (tc == `OK_Utf8') utf8_text_report(pr, e) 

	return(txt_1file_summary(pr, fn, tc, e))
}


void txt_report(`ProblemS' pr, `TextExamplesS' e)
{
	`RS'	i, l
...
void utf8_text_report(`ProblemS' pr, `TextExamplesS' e) 
{
	`RS'	i, nex
	`RS'	j
	`RS'	n
