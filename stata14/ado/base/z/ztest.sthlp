...
{vieweralsosee "[PSS] power onemean" "help power onemean"}{...}
{vieweralsosee "[PSS] power twomeans" "help power twomeans"}{...}
{vieweralsosee "[PSS] power pairedmeans" "help power pairedmeans"}{...}
...
{p2colset 5 18 20 2}{...}
{p2col:{manlink R ztest} {hline 2}}z tests (mean-comparison tests, known variance){p_end}
{p2colreset}{...}
...
variances.  In the first form,
{opt ztest} tests that {varname} has a mean of {it:#}.
In the second form, {opt ztest} tests that {it:varname} has the same mean
within the two groups defined by {it:{help varlist:groupvar}}.
In the third form, {opt ztest} tests that {it:varname1} and {it:varname2}
have the same mean, assuming unpaired data.
In the fourth form, {opt ztest} tests that {it:varname1} and {it:varname2}
...
{pstd}
{cmd:ztest} and {cmd:ztesti} store the following in {cmd:r()}:
...
{synopt:{cmd:r(sd)}}one-population standard deviation or two-population common standard deviation{p_end}
{synopt:{cmd:r(sd_d)}}standard deviation of the differences between paired observations{p_end}
{synopt:{cmd:r(corr)}}correlation between paired observations{p_end}
{synopt:{cmd:r(sd_1)}}standard deviation for group one{p_end}
{synopt:{cmd:r(sd_2)}}standard deviation for group two{p_end}
{synopt:{cmd:r(mu_1)}}sample mean for group one{p_end}
{synopt:{cmd:r(mu_2)}}sample mean for group two{p_end}
