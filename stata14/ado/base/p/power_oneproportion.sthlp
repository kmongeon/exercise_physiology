...
{viewerjumpto "Remarks" "power_oneproportion##remarks"}{...}
...
{p2colset 5 34 36 2}{...}
{p2col :{manlink PSS power oneproportion} {hline 2}}Power analysis for a one-sample
...
proportion for given sample size, power, and the null proportion.  Also see
{manhelp power PSS} for a general introduction to the {cmd:power} command
...
{mansection PSS poweroneproportionRemarksandexamplesex7:example 7}
...
Also see the {mansection pss poweroneproportionSyntaxcolumn:column} table in
{bf:[PSS] power oneproportion} for a list of symbols used by the graphs.
...
{pstd}
The following option is available with {cmd:power oneproportion} but is not
...
{marker remarks}{...}
{title:Using power oneproportion}
...
{mansection PSS poweroneproportionRemarksandexamplesex7:example 7} for
details.
...
specifying the {cmd:init()} option. See {manhelp power PSS} for the
...
{synopt:{cmd: r(init)}}initial value for the sample size or proportion{p_end}
