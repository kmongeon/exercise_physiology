{smcl}
{* *! version 1.0.8  14oct2016}{...}
{viewerdialog power "dialog power_onemean"}{...}
{vieweralsosee "[PSS] power onemean" "mansection PSS poweronemean"}{...}
{vieweralsosee "" "--"}{...}
...
{viewerjumpto "Remarks" "power_onemean##remarks"}{...}
...
{p2colset 5 28 30 2}{...}
{p2col :{manlink PSS power onemean} {hline 2}}Power analysis for a one-sample
mean test{p_end}
{p2colreset}{...}
...
{synopt :{opt fpc}}FPC as population size{p_end}
{synopt:}FPC as sampling rate{p_end}
...
the null mean.  Also see {manhelp power PSS} for a general introduction to the
...
sampling rate, n/N, where N is the total number of units in
the population.  If {cmd:fpc()} has values greater than n, it is
interpreted as a population size.  It is an error for {cmd:fpc()} to have
values between 1 and n or to have a mixture of sampling rates and
population sizes.
...
Also see the {mansection pss poweronemeanSyntaxcolumn:column} table in
{bf:[PSS] power onemean} for a list of symbols used by the graphs.
...
{pstd}
The following option is available with {cmd:power onemean} but is not
...
{mansection PSS unbalanceddesignsRemarksandexamplesFractionalsamplesizes:{it:Fractional sample sizes}} in
{bf:[PSS] unbalanced designs} for an example.  The {cmd:nfractional} option is
...
{cmd:init()} option. See {helpb power:[PSS] power} for the descriptions of
...
{synopt:{cmd: r(init)}}initial value for the sample size or mean{p_end}
