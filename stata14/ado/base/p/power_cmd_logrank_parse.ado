...
program _power_logrank_test_parse
	...
	if ("`hratio'`lnhratio'"!="") {
		if ("`solvefor'"=="esize") {
			...
			di as err "{p}only one of experimental-group " ///
				"survival probability, {bf:hratio()}, or " ///
				"{bf:lnhratio()} is allowed {p_end}"
			exit 198
		}	
	}
	if (`"`hratio'"'!= "") {
	...
	if `"`simpson'"' != "" {
		if `"`st1'"' != "" {
			di as err 	///
	"{bf:st1()} and {bf:simpson()} may not be combined"
			exit 198
		}
		if `"`solvefor'"' == "esize" {
			di as err "{bf:simpson()} is not allowed with " ///
...
exit 198
		...
		if _rc {
			di as err "{p}{bf:simpson()}: values of the survivor " ///
	...
	if `"`st1'"' != "" {
		if `"`solvefor'"' == "esize" {
			di as err "{bf:st1()} is not allowed with " ///
				  "effect-size computation"
			exit 198
		}
		tokenize `"`st1'"'
		...
		if _rc {
			di as err `"{bf:st1()}: time variable {bf:`tvar'} must "'	///
				   "contain nonnegative values"
			exit 198
		}
		cap confirm numeric variable `surv1'
		if _rc {
			di as err `"{bf:st1()}: survival variable {bf:`surv1'} "'	///
				  "must contain values between 0 and 1"
			exit 198
		}
		...
		cap assert r(min)>=0
		if _rc {
			di as err `"{bf:st1()}: time variable {bf:`tvar'} "' ///
				   "must contain nonnegative values"
			exit 198
		}
		cap assert r(max)-r(min) > 0
		if _rc {
			di as err "{bf:st1()}: length of the accrual " ///
		...
		if _rc {
			di as err ///
		"{bf:st1()}: survival variable {bf:`surv1'} " ///
		"must contain values between 0 and 1"
			exit 198
		}
		...
		if _rc {
			di as err "{bf:st1()}: values of the survivor "	///
