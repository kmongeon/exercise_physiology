...
{viewerjumpto "Remarks" "power_twoproportions##remarks"}{...}
...
{p2colset 5 35 37 2}{...}
{p2col :{manlink PSS power twoproportions} {hline 2}}Power analysis for a
two-sample proportions test{p_end} {p2colreset}{...}
{p2colreset}{...}
...
for {cmd:diff()}{p_end}
{p2coldent:* {opth rr:isk(numlist)}}relative risk, {it:p2/p1}; synonym for
{cmd:ratio()}{p_end}
...
control-group proportion.  Also see {manhelp power PSS} for a general
...
Also see the {mansection pss powertwoproportionsSyntaxcolumn:column} table in
{bf:[PSS] power twoproportions} for a list of symbols used by the graphs.
...
{pstd}
The following option is available with {cmd:power twoproportions} but is not
...
{marker remarks}{...}
{title:Using power twoproportions}
...
{mansection PSS powertwoproportionsRemarksandexamplesex8:example 8} for
details.
...
{mansection PSS powertwoproportionsRemarksandexamplessub1:{it:Alternative ways of specifying effect}}
in {bf:[PSS] power twoproportions}.
