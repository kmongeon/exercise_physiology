...
{vieweralsosee "[PSS] power repeated" "help power repeated"}{...}
...
{viewerjumpto "Remarks" "power_pairedmeans##remarks"}{...}
...
{p2colset 5 32 34 2}{...}
{p2col :{manlink PSS power pairedmeans} {hline 2}}Power analysis for a two-sample paired-means test{p_end}
{p2colreset}{...}
...
If {cmd:fpc()} has values greater than n, it is interpreted as a population
size.  It is an error for {cmd:fpc()} to have values between 1 and n or to
have a mixture of sampling rates and population sizes.
...
Also see the {mansection pss powerpairedmeansSyntaxcolumn:column} table in
{bf:[PSS] power pairedmeans} for a list of symbols used by the graphs.
...
{synopt:{cmd: r(init)}}initial value for the sample size or target mean difference{p_end}
