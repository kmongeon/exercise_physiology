...
group relative to the control group, N2/N1, for power or effect-size
determination for two-sample tests.  The default is {cmd:nratio(1)}, meaning
equal allocation between the two groups.

{phang}
{cmd:compute(n1}|{cmd:n2)} requests that the {cmd:power} command compute one
of the group sample sizes given the other one instead of the total sample size
for two-sample tests.  To compute the control-group sample size, you must
specify {cmd:compute(n1)} and the experimental-group sample size in
