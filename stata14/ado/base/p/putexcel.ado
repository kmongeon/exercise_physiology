	...
	if (_caller()<14.1) {
		...
		if ("`1'" == "set") {
			...
			syntax using / [, REPLACE MODify		///
				SHeet(string asis) locale(string)]
...
{
	transmorphic		t
	string scalar		tmpstr, token, sheet, rev
	...
	if (bsubstr(sheet, 1, 1) == `"""') {
		sheet = subinstr(sheet, `"""', "", 1)
	}

	rev = strreverse(sheet)
	if (bsubstr(rev, 1, 1) == `"""') {
		sheet = strreverse(subinstr(rev, `"""', "", 1))
	}
...
// ============================================================================
// Parsing cellexplist --- (cell)=(exp)|matrix(exp)|resultset
// ============================================================================
...
void putexcel_get_cell_info(`CEI' cei, class xl scalar xl_class,
        real scalar is_xlsx)
{
...
void putexcel_is_format(`EI' ei, class xl scalar xl_class,
	real scalar is_new_syntax)
{
...
void putexcel_check_nformat(`EI' ei, class xl scalar xl_class)
{
...
void putexcel_check_border(string scalar tmpstr, `BI' bi, 
	class xl scalar xl_class, real scalar is_new_syntax)
{
	transmorphic		t
...
void putexcel_write_format(`CEI' cei, class xl scalar xl_class)
{
	real scalar		fmtid
	real rowvector		rows, cols
	string scalar		fmt, border, style, color, err
	string matrix		str_temp
	...
	if (xl_class.get_last_error()) {
		xl_class.close_book()
		errprintf("could not write %s format to file\n", err)
		exit(691)
	}
}

void putexcel_write_format_new(real rowvector rows, real rowvector cols,
	`FIN' fin, class xl scalar xl_class, string scalar sheet)
{
	string scalar		border, style, color, err
...
void putexcel_write_picture(`CEI' cei, class xl scalar xl_class)
{
	real scalar		ul_col, ul_row
	...
	if (xl_class.get_last_error()) {
		xl_class.close_book()
		errprintf("could not write picture to file\n")
		exit(691)
	}
}
...
real rowvector putexcel_get_col_row_from_cell(class xl scalar xl_class,
	string scalar cell)
{
	real rowvector		col_row
...
void putexcel_check_cell_range(real scalar is_xlsx, class xl scalar xl_class,
...
{
	string scalar			ftype, fmode, fmtmode, s_replace
