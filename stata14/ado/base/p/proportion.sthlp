...
{synopt :{cmd:citype(logit}|{cmdab:norm:al}{cmd:)}}method to compute limits of
...
{phang}
{cmd:citype(logit}|{cmd:normal)} specifies how to compute the limits of
confidence intervals.

{phang2}
{cmd:citype(logit)}, the default, uses the logit transformation to compute the
limits of confidence intervals.

{phang2}
{cmd:citype(normal)} uses the normal approximation to compute the limits of
confidence intervals.
