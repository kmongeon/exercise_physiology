...
the column is byte based.  The number of columns means number of bytes in the
file.  The text file {it:filename} is treated as a stream of bytes, no
{mansection U 12.4.2.3Encodings:encoding} is assumed.
If string data is encoded as ASCII or UTF-8, it will be imported correctly.
