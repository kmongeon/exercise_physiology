...
{p2colset 5 22 24 2}{...}
{p2col :{manlink IRT irt pcm} {hline 2}}Partial credit model{p_end}
{p2colreset}{...}
...
{synopt :{help irt_pcm##display_options:{it:display_options}}}control
INCLUDE help opt_irt_display

{syntab:Integration}
{synopt :{cmdab:intm:ethod(}{help irt_pcm##intmethod:{it:intmethod}}{cmd:)}}integration method{p_end}
INCLUDE help opt_irt_int

{syntab:Maximization}
{synopt :{it:{help irt_pcm##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}

{synopt :{cmdab:startv:alues(}{it:{help irt_pcm##startvalues:svmethod}}{cmd:)}}method for obtaining starting values{p_end}
...
{p 4 6 2}
See {manhelp irt_pcm_postestimation IRT:irt pcm postestimation} for features
...
{pstd}
{cmd:irt pcm} fits partial credit models to ordinal items.  In the partial
credit model, items vary in their difficulty but share the same discrimination
parameter.

{pstd}
{cmd:irt gpcm} fits generalized partial credit models (GPCMs) to ordinal
items. In the GPCM, items vary in their difficulty and discrimination.
...
{pstd}
{cmd:irt pcm} stores the following in {cmd:e()}:
...
{synopt:{cmd:e(k_items1)}}number of items in first IRT model group{p_end}
{synopt:{cmd:e(k_out}{it:#}{cmd:)}}number of categories for the {it:#}th item, ordinal{p_end}
...
{synopt:{cmd:e(model1)}}{cmd:pcm}{p_end}
{synopt:{cmd:e(items1)}}names of items in first IRT model group{p_end}
{synopt:{cmd:e(n_cuts1)}}numlist of cuts in first IRT model group{p_end}
...
{synopt:{cmd:e(out}{it:#}{cmd:)}}categories for the {it:#}th item, ordinal{p_end}
