...
// Code 99 ---------------------------------------------------------------
		...
		if "`list'" != "" | "`summary'" != "" & `bad_codes' > 0 {
			...
			di _n as text "Summary of invalid and undefined codes"
				...
				list `varlist' count problem, noobs separator(0)
			}
			restore
		}
	}
	...
	if `"`generate'"' != "" {
		...
		qui label define __icd_				///
			...
			99 "Code not defined"			///
			, replace
		qui label values `generate' __icd_
...
program icd10_generate, rclass
	...
	syntax varname [if] [in] [,					///
		...
		full addcode(string) long end NODOTs pad		///
	...
	if "`version'" != "" {
		icd10_check_version "`version'"
		if "`category'" != "" {
di as err "option {bf:vernum} cannot be combined with option {bf:category}"
			exit 198
		}
		if "`range'" != "" {
di as err "option {bf:vernum} cannot be combined with option {bf:range()}"
			exit 198
		}
		if "`short'" != "" {
di as err "option {bf:vernum} cannot be combined with option {bf:short}"
			exit 198
		}
...
di as err "option {bf:full} only allowed with option {bf:description}"
	...
	if "`description'" != "" {
		...
		local rules "maxlength(4) dotrule(3)"
	}
	...
	icd generate `newvar' = `varlist' `if' `in',			///
		...
		`addcode' `full' `rules' `nodots' `pad' `short'
end
...
program icd10_search, rclass
	syntax anything(id="search text") [, or 			///
		version(numlist >0 integer min=1) year(string)]
	...
	icd search `anything', using("__icd10") `or' `vn'
	ret scalar N =r(N)
end
