...
{
	real scalar		rc, i
	string scalar		varname, vtype, cmd, vars, obs
...
*/
	...
	cmd = pr.s_version +
		" javacall com.stata.StImportDelimited main"
	...
	vars = strtrim(strofreal(st_nvar(), "%20.0fc"))
	obs = strtrim(strofreal(st_nobs(), "%20.0fc"))
	if (st_nvar() == 1) {
		printf("{text}(%s var, %s obs)\n", vars, obs)
	}
	else {
		printf("{text}(%s vars, %s obs)\n", vars, obs)
	}
}
