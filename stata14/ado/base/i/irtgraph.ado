...
program _iif
	...
	if `ti' & !missing("`anything'") {
		di "{err}varlist not allowed with option {bf:tif}"
		exit 198
	}
...
program _gr_main, rclass
	...
	if missing("`icc'") {
		if `glbcc' {
			di "{err}option {bf:bcc} not allowed"
			exit 198
		}
		if `glccc' {
			di "{err}option {bf:ccc} not allowed"
			exit 198
		}
	}
	...
	if missing("`ilist'") {
		di "{err}varlist evaluated to an empty list, nothing to draw"
		exit 198
	}
...
program _plot_icc, rclass
	...
	if `icc' | `tcc' | `iinfo' {
		if `se_add' {
di "{err}option {bf:se()} allowed only with irtgraph tif"
exit 198
		}
	}
	...
	syntax anything [, 						  ///
		...
		score_ix(integer 0) theta_ix(integer 0) *]
	...
	if !`tcc' {
		if `score_ix' {
			di "{err}option {bf:scorelines()} not allowed"
			exit 198
		}
		if `theta_ix' {
			di "{err}option {bf:thetalines()} not allowed"
			exit 198
		}
	}
	...
	foreach i of local ilist {
		...
		local model: word `pos' of `e(model_list)'
		...
		if `md' local abc `abc' `md' `=`m'[1,1]' 0 0 0
		...
		forvalues j = 1/`cuts' {
			...
			local abc `abc' `md' `w' `a' `b' `c'
		}
		local y_max = `y_max' + `m'[`cuts'+1,1]
	}
	...
	local VAR 0			// tempvar counter for y's

	// for iinfo, need to recalculate # of plots required
	if `iinfo' {
		local iif_k : list sizeof ilist
		forvalues i=1/`iif_k' {
			local ++VAR
			tempvar y`VAR'
	...
	forvalues i = 1/`k_0' {
		...
		if `iinfo' {
			local p : list posof "`tmp'" in ilist
			local y `y`p''
			local iif_ops`p' `twopts' `opts'
		}
		else if `tinfo' | `tcc' {
			local y `YY'
		}
		...
		__get_f, model(`model') item(`item') coefs(`coefs') 	///
			bases(`bases') lasts(`lasts')			///
			ix(`i') cuts(`cuts') icc(`icc')			///
		...
		local flist `macval(flist)' ///
			( line `y`i'' `Theta' , `twopts' `opts' `lstyle')
		...
		if `hasyglloc' | `hasyloc' {
			...
			if "`model'"=="3pl" {
				local ylist_3pl `ylist_3pl' `=`: di `b_hi'''
				local xlist_3pl `xlist_3pl' `=`: di `b'''
				local items_3pl `items_3pl' `item'
			}
			else local ylist `ylist' `=`: di `b'''
	...
	if `iinfo' {
		...
		local LL 1
		foreach i of local ilist {
			local p : list posof "`i'" in ilist
			local lstyle lstyle(p`sty')
			local leglab `leglab' label(`sty' "`i'")
			local flist `macval(flist)' ///
				( line `y`p'' `Theta' , `iif_ops`p'' `lstyle')
			local ++sty
			if `sty' > 15 local sty 1
			local ++LL
			label var `y`p'' "`i'"
			if `tinfo' qui replace `YY' = `YY' + `y`p''
		}
		local DD = `LL'-1
	}
	...
	if `tcc' {
		...
		local save_names `save_names' tcc
	}
	...
	local oneitem = (`k_0'==1 | `iif_k'==1)

	if missing(`"`title'"') & missing(`"`intitle'"') {
		if `tcc' {
			local title title("Test Characteristic Curve")
		}
		else if `tinfo' {
			...
			if `oneitem' local title ///
				title("Item Information Function for `ilist'")
			...
			if !`oneitem'*`bini'+!`oneitem'*(`cati'|`bcci') > 1 ///
				local pre
			if `oneitem' local title ///
			    title("`pre' Characteristic Curve for `s(ilab)'")
...
di "{err}only one of {bf:blocation} or {bf:blocation()} is allowed"
exit 198
	}
	if !missing("`plocation1'") & !missing("`plocation2'") {
di "{err}only one of {bf:plocation} or {bf:plocation()} is allowed"
exit 198
	}
	...
	if !missing("`xlocations1'") & !missing(`"`xlocations2'"') {
	      di "{err}option {bf:thetalines} can only be specified once `sub'"
	      exit 198
	}
	...
	if !missing("`ylocations1'") & !missing(`"`ylocations2'"') {
	      di "{err}option {bf:scorelines} can only be specified once `sub'"
	      exit 198
	}
...
program __check_locations_opts, sclass
	...
	if !missing(`"`options'"') {
		di "{err}option {bf:locations()}: `options' not allowed"
		exit 198
	}
...
program __cc_check
	...
	if !missing("`ccc'") {
		if !missing("`bcc'") {
			di "{err}only one of {bf:bcc} or {bf:ccc} is allowed"
			exit 198
		}
		if !missing("`blocation1'") {
			di "{err}only one of {bf:blocation} or "	///
				"{bf:ccc} is allowed"
			exit 198
		}
		...
		if !missing("`plocation1'") {
			di "{err}only one of {bf:plocation} or "	///
				"{bf:ccc} is allowed"
			exit 198
		}
...
program __ii_check
	...
	if !missing("`blocation1'") {
		di "{err}option {bf:blocation} not allowed"
		exit 198
	}
	if !missing("`blocation2'") {
		di "{err}option {bf:blocation()} not allowed"
	...
	if !missing("`plocation1'") {
		di "{err}option {bf:plocation} not allowed"
		exit 198
	}
	if !missing("`plocation2'") {
		di "{err}option {bf:plocation()} not allowed"
...
program __get_axis_lab, sclass
	...
	if _rc {
di `"{err}option {bf:`axis'(`anything')} invalid;"'
di `"{err}{p 6 6 2}{bf:`anything'} is not allowed with {bf:irtgraph}{p_end}"'
exit 198
	}
...
program __get_f, sclass
	...
	if inlist("`model'","1pl","2pl") {
		...
		local a = `coefs'[1,colnumb(`coefs',"`it':`ap'")]
		local b = `coefs'[1,colnumb(`coefs',"`it':`bp'")]
		local c = 0
	...
	if inlist("`model'","3pl") {
		...
		local a = `coefs'[1,colnumb(`coefs',"`it':`ap'")]
		local b = `coefs'[1,colnumb(`coefs',"`it':`bp'")]
		local c = `coefs'[1,colnumb(`coefs',"`it':`cp'")]
		if missing(`c') local c = 0
	...
	if inlist("`model'","grm") {
		...
		if `k0'==0 local p0 1
			...
			local a0 = `coefs'[1,colnumb(`coefs',"`it':`ap'")]
			local b0 = `coefs'[1,colnumb(`coefs',"`it':`k0'.`bp'")]
		...
		if `k0'==`cuts' local p1 0
			...
			local a1 = `coefs'[1,colnumb(`coefs',"`it':`ap'")]
			local b1 = `coefs'[1,colnumb(`coefs',"`it':`k1'.`bp'")]
	...
	if inlist("`model'","pcm","gpcm","rsm") {
		...
		forvalues i = 1/`cuts' {
			local j = `i'-1
			local a`i' = `coefs'[1,colnumb(`coefs',"`it':`ap'")]
			local b`i' = `coefs'[1,colnumb(`coefs',"`it':`i'.`bp'")]
			local z`i' `a`i''*(\`x' -`b`i'')
	...
	if inlist("`model'","nrm") {
		...
		foreach i of local levels {
			local ++j
			local a`i' = `coefs'[1,colnumb(`coefs',"`it':`j'.`ap'")]
			local b`i' = `coefs'[1,colnumb(`coefs',"`it':`j'.`bp'")]
			local z`i' `a`i''*(\`x'-`b`i'')
			local e`i' exp(`z`i'')
	...
	if `icc' {
		...
		if "`sign'" == "=" local vlab "ICC for Pr(`it'=`op')"
		else 		   local vlab "ICC for Pr(`it'>=`op')"
		
		local lab "Pr(`it'`sign'`op')"
