...
{
	real scalar i, j, k, i1, j1, n, sij, lnsig, athrho
...
{
	real scalar i, i1, j, j1, j2, k, k1, k2, kv, keq, n, d, rs
	...
	ba1 = ba[|1\k|]
	/* Dlf/Dv							*/
	Dv = dlf:*_ivfprobit_DxbDv(xb,R,C,c)
	if (trans) {
		S[|1,k1\n,k2|] = Dv:*(s':/cosh(ba1):^2)
	}
	else {
...
real matrix _ivfprobit_DxbDv(real colvector xb, real matrix R, real matrix C,
		real colvector c)
{
	real scalar rs, rs2
	real colvector cV
...
{
	real scalar i, k1, k2, keq, ec, scrsonly
	real rowvector b
	real matrix iterms, eqinfo, scores, C
	...
	iterms = st_matrixcolstripe_term_index(sb)
	...
	if (strlen(vcetype)) {
		...
		else if (strmatch(vcetype,"robust")) {
			valueid = "log pseudolikelihood"
		}
	}
	else if (!binary) {
