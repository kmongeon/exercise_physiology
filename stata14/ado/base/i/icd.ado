...
program icd_check, rclass
	...
	if "`generate'" != "" & "`list'" != "" {
di as err `"may not specify both options {bf:generate()} and {bf:list}"'
		exit 198
	}

	if "`generate'" != "" & "`summary'" != "" {
di as err `"may not specify both options {bf:generate()} and {bf:summary}"'
		exit 198
	}
	if "`list'" != "" & "`summary'" != "" {
di as err `"may not specify both options {bf:summary} and {bf:list}"'
		exit 198
	}
...
di as error  `"variable {bf:`key'} not found in file {bf:`using'}"'
	...
	if "`quickcheck'" != "" {
		...
		if "`list'" != "" | "`summary'" != ""{
			...
			di _n as text "Summary of invalid and undefined codes"
				list `varlist' count __prob, noobs separator(0)
			}
		}
	}
...
program icd_clean
	...
	if "`generate'" == "" & "`replace'" == "" {
di as err "you must specify either option {bf:generate()} or {bf:replace}"
		exit 198
	}
...
di as err `"option {bf:generate()} cannot be combined with option {bf:replace}"'
	...
	if `"`dotrule'"' == "" {
		...
		if `"`dots'"' == "" {
			di as err "you must specify option {bf:dot} "	///
	...
	quietly {
		...
		if "`dots'" != "" ! {
			...
			local len `maxlength' + 1
		}
		else {
			local len `maxlength'
			replace `c' = subinstr(`c', ".", "", .) if `touse'
		}
		local spaces = uchar(32) * `maxlength'
		...
		local len = max(8,					///
			cond(length("`varlist'")>r(max),		///
				length("`varlist'"), r(max)) + 1)
		count if `varlist' != `c' & `touse'
//		local ch = r(N)
//		local s = cond(`ch'==1, "", "s")
		if "`generate'" != "" {
			gen `generate' = `c' if `touse'
			compress `generate'
...
program icd_generate, rclass
	...
	if `nopt' != 1 {
		di as err "you must specify one of options {bf:category}, " ///
			"{bf:short}, {bf:description}, or {bf:range()}"
		exit 198
	}
	if "`category'" == "" {
		if `"`catrule'"' != "" {
...
program icd_gen__cat_short
	...
	quietly {
		...
		if "`short'"!="" {
			// just for icd10
			gen str4 `new'=subinstr(`userv', ".","",1)	///
				if `touse'
		}
		else {
			gen str`cat_l' `new'=substr(`userv',1,`cat_l')	///
				if `touse'
		}
	}
end
...
program icd_gen__descr
	...
	quietly {
		...
		if "`nodots'" == "" {
			...
			local dots "dotrule(`dotrule') dots"
		}
...
program P_ilist, sclass		// parse/execute an icd10rangelist (ilist)
	...
	while `"`tok'"'!="" & `"`tok'"' != `"`term'"' {
		...
		if `"`nxttok'"' == "*" {
			...
			if `"`range'"'=="-" | `"`range'"' == "/" {
				di as err `"invalid range() syntax"'
				exit 198
			}
			local list `"`list' `tok'*"'
		}
...
program icd_search, rclass
	...
	quietly  {
		...
		while `"`tok'"'!=""  {
			if `i' == 1 {
local myif `"`myif' strpos(`descvar',`"`tok'"')"'
			}
			else {
local myif`"`myif' `stype' strpos(`descvar',`"`tok'"')"'
			}
