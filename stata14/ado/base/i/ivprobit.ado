...
program define Estimate, eclass byable(recall) sortpreserve
	...
	syntax [if] [in] [fw pw iw / ] , [ Mle TWOstep first noLOg asis ///
	...
	if "`estimator'" == "twostep" {
		CheckTwostepOpts, `robust' cluster(`cluster') `log'          ///
	...
	if "`estimator'" == "twostep" {
		...
		MLE14, lhs(`lhs') exog(`exog') inst(`inst') end1(`end1') ///
			touse(`touse') wtype(`weight') wvar(`wvar')      ///
			vv(`version') cnt(`exog_ct' `end1_ct' `inst_ct') ///
			`log' xb(`xb') mlopts(`mlopts') `from' `clusopt' ///
			scores(`svlist') `doopt' exogname(`exogname')    ///
			end1name(`end1name') instname(`instname')
	}
	else {
		MLE, lhs(`lhs') exog(`exog') inst(`inst') end1(`end1') ///
			touse(`touse') wtype(`weight') wvar(`wvar')    ///
			cnt(`exog_ct' `end1_ct' `inst_ct') `log'       ///
	...
	if "`twostep'"=="" & `version' > 14.0 {
		local jd = 0
		local k = `end1_ct' + 1
		local yvars `lhs' `end1'
		forvalues i=1/`k' {
			local yi : word `i' of `yvars'
			forvalues j=`=`i'+1'/`k' {
				local yj : word `j' of `yvars'
				ereturn hidden local diparm`++jd' ///
					athrho`j'_`i',            ///
					label("corr(e.`yj',e.`yi')")  ///
					function(tanh(@))         ///
					derivative(1/cosh(@)^2)
			}
		}
		forvalues i=2/`k' {
			local yi : word `i' of `yvars'
			ereturn hidden local diparm`++jd' lnsigma`i', ///
				label("sd(e.`yi')") function(exp(@))    ///
				derivative(exp(@))
		}
	}
...
program define MLDisplay
	...
	local v = cond(missing(e(version)),0,e(version))
	_coef_table_header
	di
	if `v' <= 14.0 {
		version 12: ///
...
program define CheckTwostepOpts
	syntax, [ robust cluster(passthru) NOLOG inititerate(passthru) ///
...
program define MLE14, eclass
	...
	syntax, lhs(string) end1(string) touse(varname) cnt(numlist)     ///
		...
		inst(string) exogname(passthru) instname(passthru) nolog ///
		mlopts(string) wtype(string) wvar(varname) from(string)  ///
		cluster(passthru) scores(string) doopt ]
...
program define MLE, eclass
	...
	tempname b kautocns
	if "`from'" != "" {
		GetFrom, `from' cnt(`cnt') depvar(`lhs')        ///
			indepvars(`end1' `exog') ivvars(`end1') ///
			ivindvars(`exog' `inst') 
		mat `b' = r(b)
		if "`verbose'" != "" {
			mat li `b', title("from()")
		...
		ivfprobit_twostep, depvar(`lhs') indepvars(`end1' `exog') ///
			ivvars(`end1') ivindvars(`exog' `inst') binary    ///
			touse(`touse') resid(`resid') wtype(`wtype')      ///
			wvar(`wvar') `verbose' `inititerate' `log'
		mat `b' = r(b)
	}
	...
	/* test of exogeneity						*/
	ExogTest, kiv(`end1_ct')
	ereturn scalar chi2_exog = r(chi2)
	ereturn scalar df_exog = r(df)
	ereturn scalar p_exog = r(p)

	WaldTest, depvar(`lhs')
	ereturn scalar df_m = r(dfm)
	ereturn scalar chi2 = r(chi2)
	ereturn scalar p = r(p)

	ereturn scalar k_aux = `end1_ct'*(`end1_ct'+1)/2 + `end1_ct'
	ereturn scalar k_eq_model = 1
	ereturn scalar endog_ct = `end1_ct'
	ereturn scalar k_autoCns = `kautocns'
...
program define ExogTest, rclass
	syntax, kiv(integer)

	tempname b V df chi2 c

	/* get rank of V subset in case of constraints			*/
	mat `b' = e(b)
	mat `V' = e(V)
	local k = `kiv' + 1
	if `k' == 2 {
		qui test [athrho2_1]_b[_cons]
		return add
		exit
	}
	local k1 = colnumb(`V',"athrho2_1:_cons")
	local k2 = colnumb(`V',"athrho`k'_1:_cons")
	mat `V' = `V'[`k1'..`k2',`k1'..`k2']
	mat `V' = syminv(`V')
	mat `b' = `b'[1,`k1'..`k2']
	mat `c' = `b'*`V'*`b''
	scalar `chi2' = `c'[1,1]
	scalar `df' = `kiv' - diag0cnt(`V')

	return scalar p = chi2tail(`df',`chi2')
...
program define WaldTest, rclass
	...
	mat `c' = `b'*`V'*`b''
	scalar `chi2' = `c'[1,1]
	scalar `dfm' = `k' - diag0cnt(`V')

	return scalar p = chi2tail(`dfm',`chi2')
	return scalar chi2 = `chi2'
	return scalar dfm = `dfm'
end

program define GetFrom, rclass
	syntax, from(string) cnt(string) depvar(varname) indepvars(string) ///
		ivvars(string) ivindvars(string)
	...
	local k = `kex' + 1 + `ken'*(`kin'+1)
	tempname b b1 b2 b3 a

	/* exogenous equation						*/
	local k1 = `kex' + `ken' + 1
	mat `b1' = J(1,`k1',0)
	local stripe1 `"`indepvars' _cons"'
...
program define Sigma, rclass
	syntax, b(name) depvar(string) 

	tempname cor sig

	local k : list sizeof depvar
	mat `cor' = I(`k')
	mat `sig' = I(`k')
	local depvar `e(depvar)'
	forvalues j=1/`k' {
		local var : word `j' of `depvar'
		local lab `lab' e.`var'
		if `j' != 1 {
