...
{p2colset 5 27 29 2}{...}
{p2col :{manlink IRT irtgraph tcc} {hline 2}}Test characteristic curve plot{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab:Plots}
{synopt :{opt sc:orelines}{cmd:(}{it:{help numlist}} [{cmd:,} {help irtgraph_tcc##refopts:{it:refopts}}]{cmd:)}}add x and y reference lines at each score value in {it:numlist}{p_end}
{synopt :{opt th:etalines}{cmd:(}{it:{help numlist}} [{cmd:,} {help irtgraph_tcc##refopts:{it:refopts}}]{cmd:)}}add x and y reference lines at each theta value in {it:numlist}{p_end}
...
{p2colreset}{...}
{p 4 6 2}
{opt scorelines()} and {opt thetaline()} can be specified multiple times.
...
{phang}
{cmd:scorelines(}{it:{help numlist}} [{cmd:,} {it:refopts}]{cmd:)}
adds x and y reference lines at each score value in {it:numlist}.

{phang}
{cmd:thetalines(}{it:{help numlist}} {cmd:,} {it:refopts}]{cmd:)}
