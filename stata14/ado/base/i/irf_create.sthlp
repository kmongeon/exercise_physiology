...
{opt irf create} is for use after fitting a model with the
   {cmd:var}, {cmd:svar}, {cmd:vec}, {cmd:arima}, or {cmd:arfima} command;
   see {helpb var:[TS] var}, {helpb svar:[TS] var svar},
   {helpb vec:[TS] vec}, {helpb arima:[TS] arima}, and
   {helpb arfima:[TS] arfima}.{p_end}
{p 4 6 2}
You must {cmd:tsset} your data before using {cmd:var}, {cmd:svar},
   {cmd:vec}, {cmd:arima}, or {cmd:arfima} and, hence, before using
   {opt irf create}; see {helpb tsset:[TS] tsset}.{p_end}
...
method previously used to fit the model, as summarized in the table below:
                                              ...
                                              Estimation command
        Saves                           {cmd:arima}  {cmd:arfima}  {cmd:var}  {cmd:svar}  {cmd:vec}
	{hline 61}
        simple IRFs                       x      x      x    x     x
