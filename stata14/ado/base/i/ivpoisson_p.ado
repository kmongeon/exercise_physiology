...
program ivpoisson_p
	...
	syntax [anything] [if] [in] [,SCores *]  /* scores undocumented*/
	if (`"`scores'"' != "") {
		if ("`e(estimator)'"!= "cfunction") {
			di as error ///
		"scores cannot be predicted with estimator `e(estimator)'"
			exit 498
		}
		marksample touse
		Ivpscores `anything' if `touse'
		exit
	}
...
syntax anything [if] [in]
	...
	foreach val in `e(endog)' {
		local typ: word `j' of `typlist'
		local vare: word `j' of `varlist'
		gen `typ' `vare' = `resid`i''*`var' if `touse'
		local abval `val'
		local labval = ustrlen("`val'")
		if (`labval' == 32) {
			local abval = usubstr("`val'",1,29) 
