...
{viewerjumpto "Menu" "import_sasxport##menu"}{...}
...
{viewerjumpto "Stored results" "import_sasxport##results"}{...}
{title:Title}
...
{opt member(mbrname)}
is a rarely specified option indicating which member of the {opt .xpt}
file is to be described.  See the description of the {opt member()} option for
...
value labels to attempt to meet the SAS XPORT restrictions, which are that
names be no more than eight bytes long and that there be no distinction
between uppercase and lowercase letters.  Note that {opt rename} does not
remove characters beyond the normal ASCII range, such as most Unicode
characters and all extended ASCII characters.  SAS may or may not support
such characters in variable labels and value labels.
...
correspond to the location that they desire.  {cmd:export sasxport} sets the
...
{pstd}
{cmd:import sasxport, describe} stores the following in {cmd:r()}:
