	...
	if "`svy'" != "" {
		...
		if "`r(wtype)'" != "" {
			...
			forval i = 1/`stages_wt' {
				...
				if "`r(weight`i')'" != "" {
					local wvarset `r(weight`i')' `wvarset'
				}
			}
			local wvarset : list retok wvarset
			local wvarset : subinstr local wvarset " " "*", all
			quietly gen double `wvar' = `wvarset'
		...
		local stages_wt = cond(missing(r(stages_wt)), 0, r(stages_wt))
		forval i = 1/`stages_wt' {
			if "`r(weight`i')'" != "" {
				local numvars	`numvars' `r(weight`i')'
			}
		}
		return add
