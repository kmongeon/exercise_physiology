...
program _coef_table_header
	...
	local kl = `.`left'.arrnels'
	local space = `kr' - `kl' - 1
	if "`e'" == "e" & inlist("`e(dispers)'", "mean", "constant") {
		if `hascrit' {
			local --space
