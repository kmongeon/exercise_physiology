...
program _stubstar2names, sclass
	...
	if index(`"`vlist'"',"*") {
		...
		if "`subcommand'" == "" {
			local oscmd ", or you can use the `outcome'() option"
		...
		if `kvars' != `nvars' {
			...
			if `problem' {
				...
				if `nvars' > 1 & "`e(depvar)'" != "" {
					...
					di as txt ///
"{phang}The current estimation results `for'have `nvars' `outcome's so you " ///
"must specify `nvars' new variables`oscmd'.{p_end}"
				}
				exit c(rc)
			}
		}
