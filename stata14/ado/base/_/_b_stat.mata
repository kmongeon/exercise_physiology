...
void _b_stat::do_gsem_extras()
{
	...
	for (eq=1; eq<=neq; eq++) {
		...
		el2 = eq_info[eq,2]
		nels = el2 - el1 + 1
(void)		_gsem_eq_block(	stripe[el1,1],
				"",
				pclassmat[el1],
				0,
				block,
...
function _b_stat::set_citype(|string scalar citype)
{
	...
	len = strlen(citype)
	if (citype == bsubstr("normal",1,max((len,4)))) {
		_citype = `ci_normal'
	}
		...
		errprintf("invalid citype() option;\n")
		errprintf("%s is not a recognized CI type\n", citype)
		exit(198)
	}
}
