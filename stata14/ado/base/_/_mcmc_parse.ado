...
program _mcmc_parse_expression, sclass
	...
	while `"`nextlab'"' != "" {
		while regexm(`"`strexpr'"', `"{`nextlab':}"') {
			local strexpr = ///
			regexr(`"`strexpr'"', `"{`nextlab':}"', `"`nextvar'"')
		}
		gettoken nextlab lablist : lablist
		gettoken nextvar varlist : varlist
	}
...
program _mcmc_parse_expand, sclass
	...
	while "`next'" != "" {
		...
		if "`paren'" == "(" {
			gettoken suffix enext: enext, parse("\ ") bind
			tokenize "`tok'", parse("\ /")
...
program _mcmc_parse_equation, sclass
	...
	while "`next'" != ")" & "`next'" != "" {
		...
		while `"`next'"' != ")" & `"`next'"' != "," & `"`next'"' != "" {
			...
			if `s(varval)' & !`s(matval)' & "`s(word)'" != "_cons" {
				local varlist = "`varlist' `s(word)'"
			}
...
program _mcmc_parse_blocks, sclass
	...
	while `"`0'"' != "" {
		...
		while `"`next'"' != "" & `"`next'"' != "," {
			...
			local allparams `"`allparams' `s(prefix)':`s(word)'"'
	...
	local ldups: list dups allparams
	if `"`ldups'"' != "" {
