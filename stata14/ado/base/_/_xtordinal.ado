...
program _xtordinal, eclass sortpreserve byable(recall)
	...
	syntax varlist(ts fv) [if] [in] [fw pw iw] , [ OFFset(varname) 	///
		noCONstant vce(passthru) INTMethod(string) noSKIP	///
		INTPoints(integer 12) OR noLOG noDISplay I(string) *]
	...
	if _rc {
	    di as err "intmethod() invalid -- {bf`intmethod'} not allowed"
	    exit 198
	}
	...
	if "`skip'" != "" {
		if "`clust'`robus'" != "" {
			di as err ///
			"robust vce not allowed with {bf:noskip} option"
			exit 198
		}
		tempname model_0
		di
	...
	`by' me`model' `m_eq', `opts' notab nohead nolr `log' xtcmd
