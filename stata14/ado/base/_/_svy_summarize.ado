...
program _svy_summarize, eclass
	...
	if replay() {
		...
		if _caller() >= 13 {
			if "`e(cmd)'" == "proportion" {
				syntax [, CITYPE(passthru) *]
				if "`citype'" == "" {
					local 0 `", `options' citype(logit)"'
				}
			}
		}
		_prefix_display `0'
...
program Estimate, eclass
	...
	if inlist("`cmd'","prop","proportion") {
		local propopts MISSing NOLABel SVYRW percent
		local type mean
