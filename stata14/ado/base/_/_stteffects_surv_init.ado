...
program define _stteffects_surv_init, rclass
	...
	if "`verbose'" != "" {
		...
		local noi noi
		local baselev allbaselevels
	}
	else local nolog nolog

	FVCompatableVlist, vlist(`survvars') tvar(`tvar') touse(`touse')
	local fsurvvars `"`s(varlist)'"'

	local ancspec = 0
	if inlist("`survdist'","weibull","gamma","lnormal") {
		FVCompatableVlist, vlist(`survshape') tvar(`tvar') ///
			touse(`touse')
		local fsurvshape `"`s(varlist)'"'
		if `s(constant)' {
			local ancspec = inlist("`survdist'","weibull", ///
				"lnormal")
			local ancopt ancillary(`fsurvshape')
		}
		else local ancopt ancillary(`fsurvshape',noconstant)

		local avlist `survshape'
	}
	if (`ancspec') local opts iterate(5)
	...
	if `ancspec' {
		...
		GetSurvAncillary, touse(`touse') sb(`sb') klev(`klev') ///
			`ancopt' 

		cap `noi' streg `fsurvvars' if `touse', time noconstant  ///
			distribution(`survdist') ancillary(`fsurvshape', ///
			noconstant) from(`sb') `wopt' `nolog' `options' 
		mat `sb' = e(b)
	}
...
program define FVCompatableVlist, sclass
