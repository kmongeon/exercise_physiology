...
class	_tab	scalar	Tab
	...
	real	scalar	_find_min_width4gsem()
					...
					string	scalar	extra)
{
	...
	if (_cmd == "sem") {
		return(_find_min_width4sem(width, matname, dolevels, extra))
	}
	else if (anyof(tokens("gsem meglm"), _cmd) |
		 anyof(tokens("gsem meglm"), _cmd2)) {
		return(_find_min_width4gsem(width, matname, dolevels, extra))
	}
	else {
...
real scalar _b_table::_find_min_width4gsem(
	real	scalar	width,
	string	scalar	matname,
	|real	scalar	dolevels,
	string	scalar	extra)
	...
	for (eq=1; eq<=neq; eq++) {
		...
		if (anyof(coef, block) & old_name!=name) {
			...
			if (block == "base") {
				...
				if (udstrlen(name) > width - 3) {
					name = abbrev(name, width-3) 
				}
				name = name + " <-"
			}
			w = udstrlen(name)
...
void _b_table::titles()
{
	...
	if (showcns) {
		matsize = cols(bmat)
		if (matsize < c("max_matsize")) {
			if (c("matsize") <= matsize) {
				hold = c("matsize")
				matsize = max((c("min_matsize"), matsize + 1))
				rc = _stata(sprintf("set matsize %f", matsize))
				if (rc) exit(rc)
			}
		}
		else {
	...
	if (hold) { 
		rc = _stata(sprintf("set matsize %f", hold))
		if (rc) exit(rc)
	}
