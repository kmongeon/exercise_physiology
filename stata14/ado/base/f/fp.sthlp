...
{p2colset 5 15 18 2}{...}
{p2col :{manlink R fp} {hline 2}}Fractional polynomial regression{p_end}
{p2colreset}{...}
...
the "best"-fitting
{mansection I Glossaryfractionalpolynomial:fractional polynomial} substituted
for {cmd:<}{it:term}{cmd:>} wherever it appears in {it:est_cmd}.
...
{mansection R fpRemarksandexamplesScaling:{it:Scaling}}
under {it:Remarks} of {bf:[R] fp} for more details.  When {cmd:scale} is
specified, values in variable {it:term} are not modified; {cmd:fp} merely
remembers to scale the values whenever powers are calculated. 
