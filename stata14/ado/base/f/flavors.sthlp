...
{pstd}
Stata/MP and Stata/SE have most of the same limits and all the same
capabilities and are intended for those who work with large datasets.  You may
have up to 32,767 variables with either.  Statistical models may have up to
11,000 variables.

{pstd}
Stata/IC is standard Stata.  Up to 2,047 variables are allowed.  Statistical
models may have up to 800 variables.

{pstd}
Stata/MP allows up to 281,474,976,710,655 observations in theory, but you
can undoubtedly run out of memory first.  Stata/SE and Stata/IC allow up to
2,147,583,647 observations, assuming you have enough memory.
...
{center:{bf:Maximum size limits for Stata/MP, Stata/SE, Stata/IC, and Small Stata}}
    ...
    {hline}
    # observations                        *   2,147,483,647   fixed at 1,200
    # variables                      32,767           2,047      fixed at 99
    Maximum matrix size ({cmd:matsize})    10,998             798               99
    # bytes in a macro       4,227,143         264,392           51,800
    # bytes in a command     4,227,159         264,408           51,816
    {hline}
{p 4 6 2}*  For Stata/MP, the maximum number of observations is
     281,474,976,710,655, and for Stata/SE, the maximum number is
     2,147,483,647. In practice, both flavors are limited by memory.
...
{pstd}
Small Stata is, by comparison to all the above, slow, but given its limits,
