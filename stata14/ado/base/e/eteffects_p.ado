...
program define eteffects_p
	...
	if `"`scores'"' == "" {
		...
		if `stat'== 2 {
			local cmon3 linear prediction on covariates and 
			label var `name1' ///
		...
		_stubstar2names `anything', nvars(5) singleok
		local varlist  = s(varlist)
		local typlist `s(typlist)'
		local score = real("`s(stub)'")
		local ns: list sizeof varlist	
		...
		if `scorest'==1 {
			label var `name1' "parameter-level score from ATE"
		...
		if `scorest'==2 {
			label var `name1' "parameter-level score from ATET"
			label var `name2' "`cmon' when `e(tvar)'=0"
			label var `name3' ///
			"equation-level score from probit treatment"
			label var `name4' "`cmon2' `e(tvar)'=0"
