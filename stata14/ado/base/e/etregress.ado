	...
	if "`twostep'" == "" {
		...
		if "`robust'" != "" | "`hascns'" == "1" | ///
		   "`e(vcetype)'" == "Robust" {
			...
			capture qui test [athrho]_b[_cons] = 0
			if (_rc) {
				qui `vv' test ///
				[athrho0]_b[_cons] = [athrho1]_b[_cons] = 0
			}
...
program define Reparm2, eclass
	...
	mat `tmp' = `b'[1,"athrho0:_cons"]
	scalar `tau0' = `tmp'[1,1]
	mat `tmp' = `b'[1,"lnsigma0:_cons"]
	scalar `lns0' = `tmp'[1,1]

	scalar `rho0' = (exp(2*`tau0')-1) / (exp(2*`tau0')+1)
	scalar `sig0' = exp(`lns0')
	scalar `lambda0' = `rho0'*`sig0'

	mat `d' =  ( `sig0'*4*exp(2*`tau0')/(exp(2*`tau0')+1)^2 , `lambda0' )
	mat `v' =  (`v'["athrho0:_cons".."lnsigma0:_cons",      /*
		*/   "athrho0:_cons".."lnsigma0:_cons"]  )
	mat `v' = `d'*`v'*`d''
	...
	mat `tmp' = `b'[1,"athrho1:_cons"]
	scalar `tau1' = `tmp'[1,1]
	mat `tmp' = `b'[1,"lnsigma1:_cons"]
	scalar `lns1' = `tmp'[1,1]

	scalar `rho1' = (exp(2*`tau1')-1) / (exp(2*`tau1')+1)
	scalar `sig1' = exp(`lns1')
	scalar `lambda1' = `rho1'*`sig1'

	mat `d' =  ( `sig1'*4*exp(2*`tau1')/(exp(2*`tau1')+1)^2 , `lambda1' )
	mat `v' =  (`v'["athrho1:_cons".."lnsigma1:_cons",      /*
		*/   "athrho1:_cons".."lnsigma1:_cons"]  )
	mat `v' = `d'*`v'*`d''
...
program define Cfunction, eclass
	...
	if ("`poutcomes'" != "") {
		...
		athrho0:_cons lnsigma0:_cons athrho1:_cons lnsigma1:_cons
		...
		local tot `indnamed' `trtindnamed' athrho:_cons lnsigma:_cons
		local notot `noindnamed' `notrtindnamed' ///
		athrho:_cons lnsigma:_cons
	}
	...
	if ("`poutcomes'" != "") {
		...
		valueid("GMM criterion Q(b)") `cvmi'
	}
	else {
		local np = ///
