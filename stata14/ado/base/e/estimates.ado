...
program estimates
	...
	local is_bayesmh = ("`e(cmd)'"=="bayesmh")
...
program EstSave_u
	...
	mata: _st_estimatessave(`"`macval(filenm)'"', `replace')
end
...
program define _bayesmh_saved
	...
	if (`"`e(filename)'"'==`"`c(bayesmhtmpfn)'"' | ///
		...
		di as err "using {bf:bayesmh, saving()} before"
		di as err "using {bf:estimates `subcmd'}"
		exit 321
	}
end
