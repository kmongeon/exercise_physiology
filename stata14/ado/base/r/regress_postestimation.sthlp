...
variables{p_end}
{synopt :{helpb regress postestimation##syntax_estat_esize:estat esize}}eta-squared and omega-squared effect sizes{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
...
{synoptline}
{synopt :{opt xb}}linear prediction; the default{p_end}
{synopt :{opt p:r}{cmd:(}{it:a}{cmd:,}{it:b}{cmd:)}}not allowed with {cmd:margins}{p_end}
...
which are equivalent to R-squared estimates.  If the option {opt omega} is
specified, {cmd:estat} {cmd:esize} reports omega-squared estimates
(Hays {help regress postestimation##H1963:1963}),
which are equivalent to adjusted R-squared estimates.  Confidence intervals for
eta-squared and omega-squared estimates are estimated by using the noncentral F
distribution (Smithson {help regress postestimation##S2001:2001}).  See
Kline ({help regress postestimation##K2004:2004}) or
...
{p2colreset}{...}
       ...
       {cmd:esize[.,4]}    omega-squared
       {cmd:esize[.,5]}    lower confidence bound for omega-squared
       {cmd:esize[.,6]}    upper confidence bound for omega-squared
       {cmd:esize[.,7]}    numerator degrees of freedom
       {cmd:esize[.,8]}    denominator degrees of freedom
       {cmd:esize[.,9]}    F statistic
...
{phang}
Kline, R. B. 2004.
{it:Beyond Significance Testing: Reforming Data Analysis Methods in}
{it:Behavioral Research}.
