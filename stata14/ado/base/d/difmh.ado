...
program difmh, rclass sortpreserve
	...
	if !missing(`"`options'"') {
		di `"{err}option {bf:`options'} not allowed"'
		exit 198
	}
	if missing("`group'") {
		di "{err}option {bf:group()} required"
		exit 198
	}
	...
	if !missing("`maxp'") {
		...
		if _rc {
			di "{err}{bf:maxp()} must be a number in (0,1)"
			exit 198
		}
		if `maxp' <= 0 | `maxp' >= 1 {
			di "{err}{bf:maxp()} must be in (0,1)"
			exit 198
		}
	}
...
program Replay
	...
	if !missing(`"`if'"') {
		di "{err}if not allowed on replay"
		exit 198
	}
	if !missing(`"`in'"') {
		di "{err}in range not allowed on replay"
		exit 198
	}
	if !missing(`"`weight'"') {
		di "{err}weight not allowed on replay"
		exit 198
	}
	if !missing(`"`group'"') {
		di "{err}option {bf:group()} not allowed on replay"
		exit 198
	}
	if !missing(`"`listwise'"') {
		di "{err}option {bf:listwise} not allowed on replay"
		exit 198
	}
	if !missing(`"`items'"') {
		di "{err}option {bf:items()} not allowed on replay"
		exit 198
	}
	if !missing(`"`total'"') {
		di "{err}option {bf:total()} not allowed on replay"
		exit 198
	}
	if "`yates'"=="noyates" {
		di "{err}option {bf:noyates} not allowed on replay"
		exit 198
	}
	if !missing(`"`options'"') {
		di `"{err}option {bf:`options'} not allowed"'
	...
	if _rc {
		di "{err}matrix {bf:r(dif)} not found"
		exit 198
	}
	capture mat `sigma' = r(sigma2)
	if _rc {
		di "{err}matrix {bf:r(sigma2)} not found"
		exit 198
	}
	local items `r(items)'
	if missing("`items'") {
		di "{err}{bf:r(items)} not found"
		exit 198
	}
