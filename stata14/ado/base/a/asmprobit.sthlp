...
{viewerjumpto "Menu" "asmprobit##menu"}{...}
...
{viewerjumpto "Stored results" "asmprobit##results"}{...}
{title:Title}
...
{synopt :{it:{help asmprobit##display_options:display_options}}}control
...
{pstd}
{cmd:asmprobit} fits a multinomial probit (MNP) model that allows
...
variance-covariance parameters of the latent-variable errors, the model allows
you to relax the independence of irrelevant alternatives (IIA) property that
is characteristic of the multinomial logistic model and that is assumed by the
MNP model fit by {helpb mprobit}.  The command requires multiple observations
for each case (individual or decision), where each observation represents an
alternative that may be chosen.
...
{pstd}
{cmd:asmprobit} stores the following in {cmd:e()}:
