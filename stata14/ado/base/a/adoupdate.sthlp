...
{pstd}
{cmd:adoupdate} checks for available updates to user-written packages.
To update packages, use {cmd:adoupdate, update}.  By default, only
...
{cmd:ssconly} is a popular option.  Many packages are available from the
    ...
    College Archive -- which is provided at {browse "http://repec.org"}.
...
{pstd}
User-written additions to Stata are called packages and can add remarkable
abilities to Stata.  User-written packages are updated by their developers, 
just as official Stata software is updated by StataCorp.
...
type {cmd:adoupdate}.  Doing this on a regular basis can help prevent errors 
that occur when accidentally running older versions of user-written packages.
...
as shown.  Code the date in either of two formats:
	   ...
	   all numeric:  {it:yyyymmdd},  for example, {cmd:20120701}

	Stata standard:  {it:ddMONyyyy}, for example, {cmd:01jul2012}
...
{pstd}
{cmd:adoupdate} stores the following in {cmd:r()}:
