...
{p2colset 5 23 25 2}{...}
{synopt :{manlink ME mepoisson} {hline 2}}Multilevel mixed-effects Poisson regression{p_end}
{p2colreset}{...}
...
{pstd}
{cmd:mepoisson} performs optimization with the original metric of variance
components.  When variance components are near the boundary of the parameter
space, you may consider using the {cmd:meqrpoisson} command,
which provides alternative parameterizations of variance components;
see {helpb meqrpoisson:[ME] meqrpoisson}.
...
{phang}
{opt nolrtest} prevents {cmd:mepoisson} from performing a likelihood-ratio test
