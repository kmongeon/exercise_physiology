...
program __estat_icc, rclass
	...
	// check that estat icc is appropriately used after proper command
	local linear 0
	local logit 0
	local probit 0
	if "`e(cmd2)'"=="melogit" local logit 1
	if "`e(cmd2)'"=="meprobit" local probit 1
	if "`e(cmd2)'"=="meglm" {
		if "`e(family)'"=="gaussian" & "`e(link)'"=="identity" {
			local linear 1
		}
		if "`e(family)'"=="bernoulli" | "`e(family)'"=="binomial" {
			if "`e(link)'"=="logit" local logit 1
			if "`e(link)'"=="probit" local probit 1
		}
	}
	if !(`linear'|`logit'|`probit') {
		error 321
	}
        if ("`e(ivars)'"== "") {
	...
	forvalues i = 1/`wc' {
		...
		capture local x = _b[var(_cons[`w']):_cons]
		if _rc {
		...
		local den `den' + _b[var(_cons[`w']):_cons]
		local num`i' `den'
	...
	if `logit' {
		...
		local den `den' + _b[var(e.`e(depvar)'):_cons]
	}
