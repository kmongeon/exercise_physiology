...
{p2colset 5 21 23 2}{...}
{p2col :{manlink TS mswitch} {hline 2}}Markov-switching regression models{p_end}
{p2colreset}{...}
...
{cmd:states(2)}{p_end}
{synopt :{cmd:switch(}[{it:varlist}][{cmd:,} {opt nocon:stant}{cmd:)}}specify variables with switching coefficients; by default, the constant term is state dependent unless {cmd:switch(, noconstant)} is specified{p_end}
...
{phang}
{cmd:switch(}[{varlist}][{cmd:, noconstant)} specifies variables whose
...
{opt from(matname)};
see {manhelp maximize R} for all options except {opt from()}, and see below
for information on {opt from()}.
