...
{viewerjumpto "Syntax" "meoprobit##syntax"}{...}
{viewerjumpto "Menu" "meoprobit##menu"}{...}
{viewerjumpto "Description" "meoprobit##description"}{...}
{viewerjumpto "Options" "meoprobit##options"}{...}
{viewerjumpto "Remarks" "meoprobit##remarks"}{...}
{viewerjumpto "Examples" "meoprobit##examples"}{...}
{viewerjumpto "Stored results" "meoprobit##results"}{...}
{title:Title}

{p2colset 5 23 25 2}{...}
{synopt :{manlink ME meoprobit} {hline 2}}Multilevel mixed-effects ordered probit
regression{p_end}
{p2colreset}{...}
...
{cmd:meoprobit} {depvar} {it:fe_equation} [{cmd:||} {it:re_equation}]
	[{cmd:||} {it:re_equation} ...] 
	[{cmd:,} {it:{help meoprobit##options_table:options}}]

{p 4 4 2}
    where the syntax of {it:fe_equation} is

{p 12 24 2}
	[{indepvars}] {ifin} [{it:{help meoprobit##weight:weight}}]
	[{cmd:,} {it:{help meoprobit##fe_options:fe_options}}]
...
{p 12 24 2}
	{it:{help varname:levelvar}}{cmd::} [{varlist}]
		[{cmd:,} {it:{help meoprobit##re_options:re_options}}]
...
{synoptline}
{syntab:Model}
{synopt :{opth cov:ariance(meoprobit##vartype:vartype)}}variance-covariance structure of the random effects{p_end}
...
{synopt :{opt nolr:test}}do not perform likelihood-ratio test comparing with ordered probit regression{p_end}
{synopt :{it:{help meoprobit##display_options:display_options}}}control
INCLUDE help shortdes-displayoptall

{syntab:Integration}
{synopt :{opth intm:ethod(meoprobit##intmethod:intmethod)}}integration method{p_end}
{synopt :{opt intp:oints(#)}}set the number of integration
(quadrature) points for all levels; default is {cmd:intpoints(7)}{p_end}

{syntab :Maximization}
{synopt :{it:{help meoprobit##maximize_options:maximize_options}}}control
...
{p 4 6 2}See {manhelp meoprobit_postestimation ME:meoprobit postestimation}
...
{pstd}
{cmd:meoprobit} fits mixed-effects probit models for ordered responses.
...
assumed to be multinomial, with success probability
determined by the standard normal cumulative distribution function.
...
{phang}
{opt nolrtest} prevents {cmd:meoprobit} from performing a likelihood-ratio
test that compares the mixed-effects ordered probit model with standard
(marginal) ordered probit regression.  This option may also be specified
...
Those that require special mention for {cmd:meoprobit}
are listed below.
...
{pstd}
The following options are available with {opt meoprobit} but are not shown in
the dialog box:
...
{pstd}Two-level mixed-effects ordered probit regression{p_end}
{phang2}{cmd:. meoprobit thk prethk cc##tv || school:}{p_end}

{pstd}Three-level mixed-effects ordered probit regression{p_end}
...
{pstd}
{cmd:meoprobit} stores the following in {cmd:e()}:
