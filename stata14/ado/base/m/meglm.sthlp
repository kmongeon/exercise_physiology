...
{p2colset 5 19 21 2}{...}
{synopt :{manlink ME meglm} {hline 2}}Multilevel mixed-effects generalized linear model{p_end}
{p2colreset}{...}
...
{synopt :{opt nolr:test}}do not perform likelihood-ratio test comparing with reference model{p_end}
...
{bf:Statistics > Multilevel mixed-effects models >}
      {bf:Generalized linear models (GLMs)}
...
{phang}
{opt nolrtest} prevents {cmd:meglm} from fitting a reference linear
regression model and using this model to calculate a likelihood-ratio test
comparing the mixed model with ordinary regression.  This option may also be
...
{pmore}
	...
	by fitting a full fixed-effects model to 
        ...
        effects.  This is the default behavior. 

{pmore}
	{cmd:startvalues(iv)} builds on {cmd:startvalues(fixedonly)} 
	by using instrumental-variable methods with generalized residuals 
        to obtain variances of random effects. 
...
{phang2}{cmd:. meglm dtlm difficulty i.group || family: || subject:, family(bernoulli) link(probit)}{p_end}
    ...
    {hline}
{pstd}Setup{p_end}
{phang2}{cmd:. use http://www.stata-press.com/data/mlmus3/schiz}{p_end}
...
{phang2}{cmd:. meglm deaths uv, exposure(expected) || nation: || region:, family(nbinomial)}{p_end}
    ...
    {hline}
{pstd}Setup{p_end}
{phang2}{cmd:. use http://www.stata-press.com/data/mlmus3/drvisits}{p_end}
...
{phang}
{browse "http://www.youtube.com/watch?v=SbwApki_BnI&feature=youtu.be":Tour of multilevel GLMs}
