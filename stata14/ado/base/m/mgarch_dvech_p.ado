...
exit 498
	...
	local isfcast : word count `nvlist'
...
local nname : subinstr local nname "`old'" "`new'", all word
			...
			if `isstub' {
				...
				foreach var of local depvars {
					local dvi : word `i' of `newlist'
					local namei : word `i' of `nvlist'
					`q' gen `type' `namei' = `dvi'
					local lab "xb prediction, `var', `dlab'"
					label variable `namei' `"`lab'"'
					local ++i
					local q  quietly
				}
			}
...
-predict- after -mgarch dvech- supports only stub* or newvarname.
-forecast- needs to pass a newvarlist to -predict-, this is what the new
local isfcast is for.  Only -xb- prediction is supported by -forecast-.
