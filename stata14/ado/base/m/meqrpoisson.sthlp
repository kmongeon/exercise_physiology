{smcl}
{* *! version 1.2.1  11feb2014}{...}
{viewerdialog meqrpoisson "dialog meqrpoisson"}{...}
{vieweralsosee "[ME] meqrpoisson" "mansection ME meqrpoisson"}{...}
{vieweralsosee "" "--"}{...}
{vieweralsosee "[ME] meqrpoisson postestimation" "help meqrpoisson postestimation"}{...}
{vieweralsosee "" "--"}{...}
...
{viewerjumpto "Syntax" "meqrpoisson##syntax"}{...}
{viewerjumpto "Menu" "meqrpoisson##menu"}{...}
{viewerjumpto "Description" "meqrpoisson##description"}{...}
{viewerjumpto "Options" "meqrpoisson##options"}{...}
{viewerjumpto "Remarks" "meqrpoisson##remarks"}{...}
{viewerjumpto "Examples" "meqrpoisson##examples"}{...}
{viewerjumpto "Stored results" "meqrpoisson##results"}{...}
{title:Title}

{p2colset 5 25 27 2}{...}
{synopt :{manlink ME meqrpoisson} {hline 2}}Multilevel mixed-effects Poisson
...
{cmd:meqrpoisson} {depvar} {it:fe_equation} {cmd:||} {it:re_equation}
	[{cmd:||} {it:re_equation} ...] 
	[{cmd:,} {it:{help meqrpoisson##options_table:options}}]
...
{synoptline}
{syntab:Model}
{synopt :{opth cov:ariance(meqrpoisson##vartype:vartype)}}variance-covariance structure of the random effects{p_end}
...
{synopt :{opt nolr:test}}do not perform likelihood-ratio test comparing with Poisson regression{p_end}
{synopt :{it:{help meqrpoisson##display_options:display_options}}}control
...
{syntab :Maximization}
{synopt :{it:{help meqrpoisson##maximize_options:maximize_options}}}control 
...
{p_end}
{synopt :{opth refine:opts(meqrpoisson##maximize_options:maximize_options)}}control
...
{p 4 6 2}See {manhelp meqrpoisson_postestimation ME:meqrpoisson postestimation}
...
{bf:Statistics > Multilevel mixed-effects models >}
     {bf:Estimation by QR decomposition > Poisson regression}
...
{pstd}
{cmd:meqrpoisson}, like {helpb mepoisson}, fits mixed-effects models for
count responses, for which the conditional distribution of the response given
the random effects is assumed to be Poisson.

{pstd}
{cmd:meqrpoisson} provides an alternative estimation method that uses the
QR decomposition of the variance-components matrix.  This method may aid
convergence when variance components are near the boundary
...
in the fixed-effects portion of the model with the coefficient constrained 
to be 1. 

{phang}
{cmd:offset(}{help varname:{it:varname_o}}{cmd:)} specifies that
...
{cmd:independent}, {cmd:exchangeable}, {cmd:identity}, and {cmd:unstructured}.
...
{phang}{opt collinear} specifies that {cmd:meqrpoisson} not omit collinear
...
{phang} 
{opt irr} reports estimated fixed-effects coefficients transformed to incidence-rate
...
displayed, not how they are estimated.  {cmd:irr} may be specified at
...
{phang}
{opt nolrtest} prevents {cmd:meqrpoisson} from performing a likelihood-ratio
...
Those that require special mention for {cmd:meqrpoisson}
are listed below.
...
(see the description {help meqrpoisson##refineopts:below}),
...
quadrature points; see the {helpb meqrpoisson##retolerance():retolerance()}
...
{phang}
{opt refineopts}{cmd:(}{it:{help meqrpoisson##maximize_options:maximize_options}}{cmd:)} controls
the maximization process during the refinement of starting values.  Estimation
in {cmd:meqrpoisson} takes place in two stages.  In the first stage, starting
...
{pstd}
The following option is available with {opt meqrpoisson} but is not shown in
the dialog box:
...
{cmd:meqrpoisson} would be of the form
...
estimation data as one group is convenient.  Toward this end, {cmd:meqrpoisson}
allows the special group designation {cmd:_all}.  {cmd:meqrpoisson} also allows
...
{mansection ME meqrlogitRemarksandexamplesex5_meqrlogit:example 5} in
{bf:[ME] meqrlogit} for more details.
...
{pstd}Four-level nested model, fit using {helpb meqrpoisson##laplace:laplace}{p_end}
...
{pstd}
{cmd:meqrpoisson} stores the following in {cmd:e()}:
