...
{viewerjumpto "Syntax" "meprobit##syntax"}{...}
{viewerjumpto "Menu" "meprobit##menu"}{...}
{viewerjumpto "Description" "meprobit##description"}{...}
{viewerjumpto "Options" "meprobit##options"}{...}
{viewerjumpto "Remarks" "meprobit##remarks"}{...}
{viewerjumpto "Examples" "meprobit##examples"}{...}
{viewerjumpto "Stored results" "meprobit##results"}{...}
{title:Title}

{p2colset 5 22 24 2}{...}
{synopt :{manlink ME meprobit} {hline 2}}Multilevel mixed-effects probit regression
{p_end}
{p2colreset}{...}
...
{cmd:meprobit} {depvar} {it:fe_equation} [{cmd:||} {it:re_equation}]
	[{cmd:||} {it:re_equation} ...] 
	[{cmd:,} {it:{help meprobit##options_table:options}}]

{p 4 4 2}
    where the syntax of {it:fe_equation} is

{p 12 24 2}
	[{indepvars}] {ifin} [{it:{help meprobit##weight:weight}}]
	[{cmd:,} {it:{help meprobit##fe_options:fe_options}}]
...
{p 12 24 2}
	{it:{help varname:levelvar}}{cmd::} [{varlist}]
		[{cmd:,} {it:{help meprobit##re_options:re_options}}]
...
{synoptline}
{syntab:Model}
{synopt :{opth cov:ariance(meprobit##vartype:vartype)}}variance-covariance structure of the random effects{p_end}
...
{synopt :{opt nolr:test}}do not perform likelihood-ratio test comparing with probit regression{p_end}
{synopt :{it:{help meprobit##display_options:display_options}}}control
INCLUDE help shortdes-displayoptall

{syntab:Integration}
{synopt :{opth intm:ethod(meprobit##intmethod:intmethod)}}integration method{p_end}
{synopt :{opt intp:oints(#)}}set the number of integration
(quadrature) points for all levels; default is {cmd:intpoints(7)}{p_end}

{syntab :Maximization}
{synopt :{it:{help meprobit##maximize_options:maximize_options}}}control
...
{p 4 6 2}See {manhelp meprobit_postestimation ME:meprobit postestimation}
...
{phang}
{bf:Statistics > Multilevel mixed-effects models > Probit regression}
...
{pstd}
{cmd:meprobit} fits mixed-effects models for binary or binomial responses.
The conditional distribution of the response given the random effects is
assumed to be Bernoulli, with success probability determined by the standard
...
{phang}
{opt nolrtest} prevents {cmd:meprobit} from performing a likelihood-ratio
test that compares the mixed-effects probit model with standard (marginal)
probit regression.  This option may also be
...
Those that require special mention for {cmd:meprobit}
are listed below.
...
{pstd}
The following options are available with {opt meprobit} but are not shown in
the dialog box:
...
{pstd}Two-level random-intercept model, analogous to {cmd:xtprobit}{p_end}
...
{pstd}
{cmd:meprobit} stores the following in {cmd:e()}:
