...
{p2colset 5 23 25 2}{...}
{synopt :{manlink ME meqrlogit} {hline 2}}Multilevel mixed-effects logistic
...
{pstd}
{cmd:meqrlogit}, like {helpb melogit}, fits mixed-effects models for
binary or binomial responses. The conditional distribution of the response
given the random effects is assumed to be Bernoulli, with success probability
determined by the logistic cumulative distribution function.

{pstd}
{cmd:meqrlogit} provides an alternative estimation method, which uses the
...
{phang}
{opt nolrtest} prevents {cmd:meqrlogit} from performing a likelihood-ratio
