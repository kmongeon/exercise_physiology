...
{p 4 8 2}
{manlink M-5 _docx*()} {hline 2} Generate Office Open XML (.docx) file 
...
{browse "http://www.stata.com/docx_styles.html"} for more discussion on styles.   
...
saves the document identified by the ID {it:dh} to the file {it:filename} on
...
{browse "http://www.stata.com/docx_styles.html"} for more discussion on
styles.   
...
{p 4 4 2}
The function is not supported on Stata for Mac running on Mac OS X 10.9
...
numeric format, {cmd:%12.0g} is used.  The first row of the table is filled
with {help matrix_rownames:matrix colnames}.  If {it:rownames} is not {cmd:0},
...
see {browse "http://www.stata.com/docx_styles.html"}.
...
{p 4 4 2}
We use the {cmd:auto dataset} in the rest of the examples. 
