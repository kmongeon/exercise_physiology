...
{p2colset 5 19 21 2}{...}
{synopt :{manlink ME mixed} {hline 2}}Multilevel mixed-effects linear regression
{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab:Model}
{synopt :{opt nocon:stant}}suppress constant term from the fixed-effects equation{p_end}
{synoptline}
...
{synopt :{opt nocon:stant}}suppress constant term from the random-effects 
equation{p_end}
...
{synopt :{opt stddev:iations}}show random-effects and residual-error parameter estimates as standard deviations{p_end}
...
{synopt :{opt nolr:test}}do not perform likelihood-ratio test comparing with linear 
regression{p_end}
...
{p 4 6 2}
{opt small} and {opt coeflegend} do not appear in the dialog box.{p_end}
...
{phang2}
{cmd:independent} allows a distinct variance for each random effect within a
random-effects equation and assumes that all covariances are 0.
...
{phang}
{opt nolrtest} prevents {cmd:mixed} from fitting a reference linear 
