...
{p 4 4 2}
Coding {cmd:version} {cmd:14.2} will not benefit us today but, in the 
...
setting the version at the top of your program:
	...
	program myfile 
		{cmd:version 14.2}      <- {it:as usual}
		...
	end

	{cmd:version 14.2}              <- {it:new}
	{cmd:mata:}
	...
	{cmd:end}
...
{p 4 4 2}
In the above, we compile {cmd:example()} by using the version 14.2 syntax of
the Mata language,
and any functions {cmd:example()} calls will be the 14.2 version of those
functions.  Setting {cmd:version} {cmd:14.2} before entering Mata ensured 
all of that.

{p 4 4 2}
In the following example, we compile using version 14.0 syntax 
and use version 14.2 functions:
...
{p 4 4 2}
In the following example, we compile using version 14.2 syntax 
and use version 14.0 functions:
