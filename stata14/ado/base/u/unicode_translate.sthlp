...
{p2colset 5 30 32 2}{...}
{p2col :{manlink D unicode translate} {hline 2}}Translate files to Unicode{p_end}
{p2colreset}{...}
...
{pstd}
{cmd:unicode} analyzes and translates .dta files and text files.  It
assumes that filenames with suffix .dta contain Stata datasets and that all
...
in this context, before Stata became Unicode aware. 
        ...
        {c |} If you have do-files, ado-files, .dta files, etc.,   {c |}
...
{pmore}
     ...
     if dataset result.dta was the result of merging
     input1.dta and input2.dta, and input1.dta encoded its string
     variables using ISO-8859-1, whereas input2.dta used JIS-X-0208.
     Such datasets are rare because if this had occurred, 
     you would have noticed when you produced result.dta. 
     The two extended ASCII encodings are simply not compatible, and
     one group or another of characters would have displayed
     incorrectly.
...
{pstd}
Stata 14 uses UTF-8, a form of Unicode, to encode strings.  Previous releases
of Stata used ASCII.  Datasets, do-files, ado-files, help files, and the like
may need translation to display properly in Stata 14.
...
variables in your .dta files and the lines in your do-files,
ado-files, and other Stata text files contain only the characters
above, there is nothing you need to do.

{pstd}
On the other hand, if your .dta files, do-files, ado-files, etc.,
...
ASCII, they will not display correctly in Stata 14.  We view that as a
significant problem, but let's assume that does not concern you.  If
you used extended ASCII for variable names, you may find it
difficult or impossible to type the untranslated name.  That would
be a problem.  Other than that, you are probably okay, or more
accurately, we cannot think of a problem even though we have tried.
We have tried because if we could think of a problem, we would have
fixed it.  Stata's data management routines have been modified and
certified to work with UTF-8.  If they receive extended ASCII, they can
mightily mess up what is displayed, but beyond that, they should
...
was named japan.dta and that the examples did not look like Japanese but
...
the detailed output for .dta files will be something like the following:
