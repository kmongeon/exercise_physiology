...
{p2colset 5 27 29 2}{...}
{p2col :{manlink D unicode locale} {hline 2}}Unicode locale utilities{p_end}
{p2colreset}{...}
...
{pstd}
The process stops at any point if the desired information is found. The ICU default
locale is usually the system locale on the machine, which you can change.
Note that on Mac OS X, the ICU default locale is usually "en_US_posix", 
which does not change even if you change the system locale from the operating
system's "Language" setting.  To see the ICU default locale, you can type
