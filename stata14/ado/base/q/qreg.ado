...
program qreg, eclass byable(recall) prop(sw mi)
	if _by() {
...
program define OptimizeParse, sclass
	syntax namelist(max=2) [, ITERate(passthru) nolog trace *]
	...
	c_local `c_opopts' `iterate' `log' `trace'
	c_local `c_opts' `"`opts'"'
end
...
void _qreg_kernel(string scalar kernel, 	///
		...
		|string scalar kvar)
{
	real colvector	xvec, val, cnd, wvec, w
