...
{marker technote2}{...}
{title:Technical note for using PDF translators}

{pstd}
When using {opt translate} to translate log, SMCL, and text files into
PDF files, some characters may not display correctly in the resulting
PDF file.  This happens when the corresponding glyph for such characters
cannot be found within the default font used by Stata.  Additional
fonts that contain the corresponding glyph(s) may need to be specified.
You can set the additional font by specifying the {opt addfonts} suboption
...
option under {it:Options for translate}.  {it:fontname} is the font name,
rather than the filename, of a Base14 font, a TrueType font with an
extension of {bf:.ttf} or {bf:.ttc}, or a Type 1 font.  If {it:fontname} refers
to a TrueType font or a Type 1 font, the corresponding TrueType or Type 1 font
file will be searched within the default font directory.  Unix (console) users
can type
...
font directories that are separated by semicolons.  If no TrueType or Type 1 font
file is found for the specified {it:fontname} under the current font
directory, an error message will be issued.  

{pstd}
When the original log, SMCL, or text files contains Unicode characters
referring to glyphs from different languages, one font is not enough to
be used to display every character because there is not one font that can
cover the whole range of Unicode characters.  In this case, you can specify a
list of fonts to build a font chain.  Those fonts are specified in
{it:fontname} and are separated by commas.  The most preferred font goes
first.  A maximum of 16 fonts can be specified.  The font chain will look at
each character in the original file.  When the glyph of the corresponding
character is available in a font, this font is selected to display this
character.  Otherwise, Stata will look up the character in the next font
within the font list until the corresponding glyph is found.  If no font in
the list is selected, the character will not display correctly in the
resulting PDF file.
